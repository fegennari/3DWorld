...
void tree::add_tree_collision_objects(int ix) {
	...
	if (tree_coll_level >= 4) {
		...
		lcolor.alpha = 1.0;
		cobj_params cpl(0.3, lcolor, TEST_RTREE_COBJS, 0, NULL, ix, /*ltid*/-1, 1.0, 0, 0);
		cpl.shadow = 0;
...
void tree::draw_tree(bool invalidate_norms) {
	...
	if (deadness < 1.0 && init_deadness < 1.0) {
		burn_leaves();
	...
	if (!sphere_in_camera_view(sphere_center, 1.1*sphere_radius, (island ? 1 : 2))) return; // don't test the mesh in island mode
	int const light(get_light());
	float const mscale(mesh_scale*mesh_scale2), dc_center(distance_to_camera(sphere_center));
	float const dist_c(mscale*DIST_C_SCALE/(do_zoom ? ZOOM_FACTOR : 1.0));
	float const dist_cs(dc_center*dist_c*(0.04/base_radius)), bs_scale(350.0*mscale/dist_c);
	...
	if (use_vbos) { // draw with branch vbos
		...
		if (branch_vbo == 0) { // create vbo
			...
			map<unsigned, unsigned> vmap;
			unsigned cix(0);

			for (unsigned i = 0; i < numcylin; i++, cix++) {
				...
				if (!prev_connect) { // new branch
					vmap.clear();
					cix = 0;
				}
				for (unsigned S = 0; S < ndiv; ++S) { // ndiv can change
					for (unsigned j = 0; j < 2; ++j) {
						unsigned const sj(S+j), s(sj%ndiv), sm1((s+ndiv-1)%ndiv);
						float const tx(CLIP_TO_01(1.0f - sj*ndiv_inv));
						vector3d const norm(vpn.n[s] + vpn.n[sm1]); // not normalized

						for (unsigned d = 0; d < 2; ++d) {
							bool const k((d^j) != 0);
							unsigned const key(cix+k + (sj<<16));
							map<unsigned, unsigned>::const_iterator it(vmap.find(key));

							if (it == vmap.end()) { // not found
								indices.push_back(data.size());
								data.push_back(vert_norm_tc(vpn.p[(s<<1)+k], norm, tx, float(k^(i&1))));
								vmap[key] = indices.back();
							}
							else { // found
								assert(it->second < data.size());
