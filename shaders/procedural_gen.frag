...
void main()
{
	...
	if (z_top_test) {
		if (use_noise_tex) {texel = get_texture_val_z_test(norm_normal, vpos);}
		else {lookup_triplanar_texture_2sz(vpos, norm_normal, tex0, tex0, tex0, tex1) * color0;}
	}
	else {
		if (use_noise_tex) {texel = get_texture_val(norm_normal, vpos);}
		else {texel = lookup_triplanar_texture(vpos, norm_normal, tex0, tex0, tex0) * color0;}
	}
	...
	if (enable_dlights) add_dlights(lit_color, vpos, epos, norm_normal, gl_Color.rgb); // dynamic lighting
