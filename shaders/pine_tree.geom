...
void main()
{
	...
	for (int j = 0; j < N_PT_LEVELS; ++j) {
		...
		for (int k = 0; k < N_PT_RINGS; ++k) {
			...
			gl_Position = gl_ModelViewProjectionMatrix * pts[0]; gl_FogFragCoord = length(gl_ModelViewMatrix * pts[0]); tc = vec2(0.0, 0.0); EmitVertex();
			gl_Position = gl_ModelViewProjectionMatrix * pts[1]; gl_FogFragCoord = length(gl_ModelViewMatrix * pts[1]); tc = vec2(1.0, 0.0); EmitVertex();
			gl_Position = gl_ModelViewProjectionMatrix * pts[3]; gl_FogFragCoord = length(gl_ModelViewMatrix * pts[3]); tc = vec2(0.0, 1.0); EmitVertex();
			gl_Position = gl_ModelViewProjectionMatrix * pts[2]; gl_FogFragCoord = length(gl_ModelViewMatrix * pts[2]); tc = vec2(1.0, 1.0); EmitVertex();
			EndPrimitive();
		}
	}
}