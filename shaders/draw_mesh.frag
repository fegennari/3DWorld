...
void main()
{
	...
	if (enable_light0) lit_color += add_light_comp_pos_smap(normalize(eye_norm), vec4(eye,1), vec4(dlpos,1), 0);
	if (enable_light1) lit_color += add_light_comp_pos_smap(normalize(eye_norm), vec4(eye,1), vec4(dlpos,1), 1);
