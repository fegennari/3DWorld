...
void main() {
	...
	if (fg_Color_vf[0].a == 0.0 || fg_Color_vf[1].a == 0.0 || fg_Color_vf[2].a == 0.0) { // transparent = disabled
		vertex_ES[gl_InvocationID] = vec3(0.0);
		level = 1;
	}
	else {
		...
		level = max(min_tess_level, min(8, tess_lod_scale*(2.5/dist)));
	}
