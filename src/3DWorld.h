...
enum {GROUND_TEX = 0, DARK_ROCK_TEX, WATER_TEX, WATER2_TEX, CLOUD_TEX, SUN_TEX, MOON_TEX, EARTH_TEX, MARBLE_TEX, SNOW_TEX,
	...
	CLOUD_RAW_TEX, SNOWFLAKE_TEX, BLUR_CENT_TEX, GRADIENT_TEX, GRASS_BLADE_TEX
};
...
void draw_fast_cylinder(point const &p1, point const &p2, float radius1, float radius2, int ndiv, bool texture, bool draw_ends=0,
...
void gen_scorch_mark(point const &pos, float radius, vector3d const &orient, int cid=-1, float init_alpha=1.0);
...
void build_cobj_tree(bool dynamic=0);
void update_cobj_tree(bool dynamic=0);
bool check_coll_line_exact_tree(point const &p1, point const &p2, point &cpos, vector3d &cnorm, int &cindex, int ignore_cobj, bool dynamic=0);
bool check_coll_line_tree(point const &p1, point const &p2, int &cindex, int ignore_cobj, bool dynamic=0);
...
int  set_true_obj_height(point &pos, point const &lpos, float step_height, float &zvel, int type, int id, bool flight, bool on_snow, bool skip_dynamic=0);
