...
string u_ship::get_info() const { // destination, homeworld?
	...
	if (specs().max_armor   > 0.0) {oss << (specs().has_health() ? "Health: " : "Armor: ") << unsigned(100.0*armor/specs().max_armor) << "%  ";}
...
bool u_ship::has_clear_line_of_fire(us_weapon const &weap, vector3d const &fire_dir, float target_dist) const {
	...
	free_obj *fobj(NULL);
	float tdist(0.0); // unused
	uobject const *line_of_fire(setup_int_query(fire_dir, target_dist, fobj, tdist, 0, line_radius));
	if (line_of_fire == NULL) return 1; // we're good to go
	assert(line_of_fire != target_obj);
...
void u_ship::fire_beam(point const &fpos, vector3d const &fdir, unsigned weapon_id, unsigned num_shots, int intersect_type) {
	...
	if (!bwp.multi_segment) {
		...
		for (unsigned i = 0; i < segments; ++i) {
			...
			if (pt[0] != pt[1]) {beam_rays.push_back(usw_ray(bw[0], bw[1], pt[0], pt[1], c[0], c[1]));}
		}
	}
}
...
void u_ship::apply_physics() {
		...
		fticks*velocity.mag() > 0.1f*radius && univ_sphere_vis(pos, 2.0f*c_radius))
	{
		...
		if (!is_distant(pos, 2.0*pscale)) {
			...
			gen_particle(PTYPE_GLOW, LT_GRAY, colorRGBA(0.75, 0.75, 0.75, 0.0), ((5*TICKS_PER_SECOND)/2), pos2, vel, psize, 0.0, alignment, 0, SMOKE_PUFF_TEX);
		}
	}
