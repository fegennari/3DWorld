	...
	if (proc_cobjs) {
		...
		for (unsigned q = 0; q < ncv; ++q) {
			...
			if (r_cobj.overlaps(bbz) && add_cobj_ok(cobj) && cobj.clip_in_2d(bbz, cztop, 0, 1, 1)) {
				cobj_z.emplace_back(cztop, cid); // still incorrect for coll polygon since x and y aren't clipped
			}
		}
...
void add_player_flashlight_light_source(float radius_scale) { // for buildings
	...
	bool const enable_shadows(camera_in_building && (camera_pos - get_camera_coord_space_xlate()).mag() < 32.0);
