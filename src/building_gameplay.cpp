...
void setup_bldg_obj_types() {
	...
	bldg_obj_types[TYPE_SERVER    ] = bldg_obj_type_t(1, 1, 1, 1, 0, 0, 1, 10000, 400.00,"server"); // too heavy for inventory
	...
	bldg_obj_types[TYPE_METAL_BAR ] = bldg_obj_type_t(0, 0, 0, 0, 1, 0, 2,  0.0,  0.0,   "metal bar"); // sometimes rat/spider collidable
...
bldg_obj_type_t get_taken_obj_type(room_object_t const &obj) {
	...
	else if (otype == TYPE_SIGN && obj.in_mall()) { // mall store sale signs are worth $1 rather than $10
		type.value  = 1.0;
		type.weight = 0.25;
	}
