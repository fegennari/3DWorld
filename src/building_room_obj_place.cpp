	...
	float zval, unsigned room_id, unsigned floor_ix, float tot_light_amt, unsigned objs_start, bool is_basement)
{
	...
	if (!is_house && (is_prison() || is_datacenter() || rgen.rand_float() < 0.5) && !room_has_stairs_or_elevator(room, zval, floor_ix)) { // allow two desks in one office
...
{
	bool const is_bed(is_bedroom(room_type)), is_pantry(!is_bed && is_house), is_freezer(!is_bed && !is_house);
...
float building_t::add_flooring(room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned flooring_type, unsigned sub_type) {
	...
	for (stairwell_t const &s : interior->stairwells) {
		...
		cube_t s_exp(s);
		s_exp.expand_in_dim(!s.dim, s.get_wall_hwidth(get_window_vspace())); // expand to clip out slivers next to exterior walls
		subtract_cube_from_cubes(s_exp, fparts, nullptr, 0, 0, 1); // no holes, no_z_test=1
	}
...
bool building_t::add_pool_room_objs(rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt) {
	float const sz_in_feet(get_one_foot()), clearance(get_min_front_clearance_inc_people());
	vect_room_object_t &objs(interior->room_geom->objs);
	...
	float tot_light_amt, unsigned objs_start, unsigned floor_ix, bool is_basement, unsigned pref_orient)
{
	...
	for (unsigned dim = 0; dim < 2; ++dim) {
		for (unsigned dir = 0; dir < 2; ++dir) {
			...
			for (unsigned num = 0; num < (more_pictures ? 2U : 1U); ++num) { // up to 2 pictures in art rooms and restaurants
				...
				for (unsigned n = 0; n < (more_pictures ? 20U : 10U); ++n) { // make 10/20 attempts to choose a position along the wall; first iteration is the center
...
bool building_t::is_light_placement_valid(cube_t const &light, room_t const &room, float pad) const {
	...
	for (stairwell_t const &s : interior->stairwells) {
		if (s.intersects(light) && s.z2() - stairs_z2_adj > light.z2() && !s.contains_cube_xy(light) && (s.has_walled_sides() || s.is_u_shape())) return 0;
	}
	...
	bool allow_rot, bool allow_mult, unsigned nx, unsigned ny, unsigned check_coll_start, vect_cube_t &lights, rand_gen_t &rgen) const
{
	...
	if (is_light_placement_valid(light, room, pad) && !overlaps_other_room_obj(light, check_coll_start)) {
		...
		for (unsigned D = 0; D < 2 && !light_placed; ++D) { // try both dims
			...
			// shift within the bounds of placement grid based on num
			float const shift_step((0.5*(room.get_sz_dim(dim) - light_cand.get_sz_dim(dim) - get_wall_thickness()))/(num*num_shifts));
