...
public:
	...
	void clear();
	void create_procedural(float mag, float freq, vector3d const &offset, bool normalize_to_1, int rseed1, int rseed2, int gen_mode, bool verbose);
...
public:
	...
	cube_t get_bcube() const {return ((tri_data[0].empty()) ? cube_t(center) : tri_data[0].get_bcube());}
