...
void usw_ray::draw() const { // use single sided cylinder with 1D blur rotated towards camera
	...
	draw_line_tquad(p1, p2, w1, w2, color1, color2, 1);
}
...
void ship_cube::draw(unsigned ndiv) const { // ndiv is unused
	...
	draw_cube(pt, sz[0], sz[1], sz[2], 0, 1);
}
...
void uobj_draw_data::draw_ehousing_pair(float length, float r1, float r2, float lcone, float dx, float dy) const {
	...
	for (unsigned i = 0; i < 2; ++i) { // draw engine housings
		draw_cylinder_nstacks(length, r1, r2, ndiv2, nstacks, 0);
		if (lcone > 0.0 && r1 > 0.0) draw_cylin_fast(r1, 0.0, lcone, ndiv2, 0, 1);
		glTranslatef(0.0, 0.0, length);
		if (lcone > 0.0 && r2 > 0.0) draw_cylin_fast(r2, 0.0, lcone, ndiv2, 0, 1);
...
void uobj_draw_data::draw_us_frigate() const {
	...
	draw_sphere_dlist(all_zeros, 1.0, ndiv, 0);
	glPopMatrix();
	color_b.do_glColor();
	glPushMatrix();
	...
	if (ndiv > 3) { // draw engines
		...
		for (unsigned i = 0; i < nengines; ++i) {
			draw_cylin_fast(0.2, 0.22, 1.1, ndiv2, 0, 1);
			glTranslatef(0.0, 0.0, 0.1);
			if (ndiv > 8) draw_circle_normal(0.0, 0.202, ndiv2, 1);
...
void uobj_draw_data::draw_us_destroyer() const {
	...
	draw_cylinder(1.7, 0.45, 0.45, ndiv, (dlights ? max(1, ndiv/4) : 1), 1, 1, 0);
	glPushMatrix();
	glScalef(1.0, 1.0, 2.0);
	draw_sphere_dlist(point(0.0, 0.0, 0.85), 0.45, ndiv, 0, 1);
	set_cloak_color(RED);
	...
	glPopMatrix();
	draw_cylindrical_section(point(0.0, 0.0, -0.1), 0.2, 0.85, 1.2, min(N_CYL_SIDES, get_ndiv((3*ndiv)/2))); // engine ring

	if (ndiv > 4) { // draw engines supports
		glPushMatrix();
		glScalef(1.0, 0.05, 0.1);
		glutSolidCube(1.8);
		glScalef(0.05, 20.0, 1.0);
		glutSolidCube(1.8);
		glPopMatrix();
	}
	if (ndiv > 6) { // draw parallel beams
		glPushMatrix();
		glTranslatef(0.0, 0.0, 0.8);
		glScalef(0.6, 0.05, 0.9);
		glutSolidCube(1.8);
		glScalef(0.05, 12.0, 1.0);
		glutSolidCube(1.8);
		glPopMatrix();
	}
	if (ndiv > 8) { // draw engines
		...
		for (unsigned i = 0; i < nengines; ++i) {
			...
			draw_cylin_fast(0.12, 0.1, 0.14, ndiv25, 0, 0);
			glPopMatrix();
		}
		set_lighted_sides(1);
	}
...
void uobj_draw_data::draw_us_cruiser(bool heavy) const {
	...
	draw_cylinder_nstacks(1.8, 0.38, 0.26, ndiv, (dlights ? max(1, ndiv/5) : 1), 0);
	glPushMatrix();
	glScalef(1.0, 1.0, 1.45);
	invert_z(); // invert for half sphere
	draw_sphere_dlist(all_zeros, 0.38, ndiv, 0, 1); // back
	glPopMatrix();
	glPushMatrix();
	glTranslatef(0.0, 0.0, 1.8);
	glScalef(1.0, 1.0, 2.12);
	draw_sphere_dlist(all_zeros, 0.26, ndiv, 0, 1); // front
	glPopMatrix();
	...
	for (unsigned i = 0; i < nengines; ++i) { // draw engine housings
		set_lighted_sides(2);
		pos2 += epos[i];
		draw_sphere_dlist(pos2, 0.16, ndiv2, 0);

		if (ndiv > 8) { // draw engines
			glPushMatrix();
			glTranslatef(pos2.x, pos2.y, (pos2.z - 0.165));
			draw_cylin_fast(0.06, 0.08, 0.03, ndiv3, 0, 0);
			glPopMatrix();
		}
		set_lighted_sides(1);
	}
	...
	if (ndiv > 5) { // draw engine struts
		...
		for (unsigned i = 0; i < nengines; ++i) {
			...
			draw_fast_cylinder(pos0, pos2, 0.1, 0.1, ndiv2, 0);
		}
		glPopMatrix();
	}
	if (heavy && ndiv >= 4) {
		...
		for (unsigned i = 0; i < 4; ++i) { // draw weapon housings
			pos2 += epos[i];
			draw_sphere_dlist(pos2, 0.1, ndiv25, 0);
		}
		glPopMatrix();
	}
	glPopMatrix(); // undo invert_z()
...
void uobj_draw_data::draw_us_enforcer() const { // could be better
	...
	gluCylinder(quadric, 0.65, 0.0, 2.4, ndiv32, (dlights ? ndiv2 : 1)); // main body
	...
	if (ndiv > 6) { // draw engines
		for (unsigned i = 0; i < nengines; ++i) {
			...
			if (ndiv > 8) draw_circle_normal(0.0, 0.11, ndiv2, 0);
			glPopMatrix();
		}
	}
	set_lighted_sides(1);
...
void uobj_draw_data::draw_us_carrier() const {
	...
	unsigned const ndiv2(get_ndiv(ndiv/2)), ndiv_dl8(dlights ? max(1, ndiv/8) : 1);
	assert(nengines == 2);
	setup_draw_ship();
	set_ship_texture(PLASTER_TEX);
	colorRGBA const ecolor(0.2, 0.2, 1.0, 1.0);
	light_engine_pair(ecolor, 0, 0.5, 0.7, 0.0, 1.3);
	draw_cube(point(0.0, 0.0, -0.32), 0.8, 0.38, 2.04, 1, (dlights ? max(1, ndiv/2) : 1), 1); // main body
	glPushMatrix();
	...
	glRotatef(-90.0, 1.0, 0.0, 0.0);
	draw_cylinder(0.18, 0.2, 0.2, ndiv, ndiv_dl8, 1); // front
	end_ship_texture();
	...
	if (ndiv > 6) {
		...
		draw_cylin_fast(0.03, 0.03, 0.1, ndiv, 1, 1);
	...
	draw_ehousing_pair(1.0, 0.13, 0.14, 0.2, -1.4, 0.0);
	glPopMatrix();
	set_ship_texture(VSTRIPE_TEX);
	glPushMatrix();
	glTranslatef(0.0, 0.19, -0.2);
	glRotatef(-90.0, 1.0, 0.0, 0.0);
	if (ndiv > 3) draw_cylinder_nstacks(0.1, 0.22, 0.22, ndiv2, ndiv_dl8, 1); // weapons turret
	...
	if (ndiv > 9) {
		...
		draw_cylin_fast(0.008, 0.005, 0.3, ndiv4, 0, 1);
...
void uobj_draw_data::draw_armageddon(mesh2d const &surface_mesh) const {
	...
	color_b.do_glColor();
	draw_cube(point(0.0, -0.68, 0.0), 0.6, 0.4, 1.2, 1, (dlights ? max(1, ndiv/3) : 1), 1); // box
	end_ship_texture();
	...
	if (ndiv > 6) {
		...
		for (unsigned i = 0; i < nbands; ++i) { // draw bands
			float const w(0.05), zpos(0.8*(2.0*crs*i/(nbands - 1.0) - crs));
			float const r(fabs(zpos)/crs), radius(xys*sqrt(1.0 - r*r));
			cgray.do_glColor();
			glPushMatrix();
			glTranslatef(0.0, 0.0, (zpos - 0.5*w));

			if (ndiv > 9) {
				draw_cylindrical_section(all_zeros, w, 0.9*radius, 1.05*radius, ndiv_c);
			}
			else {
				draw_cylin_fast(1.05*radius, 1.05*radius, w, ndiv_c, 0, 1);
			}
	...
	draw_ehousing_pair(0.9, 0.12, 0.12, 0.15, -0.84, 0.0);
	glPopMatrix(); // undo invert_z()
...
void uobj_draw_data::draw_defsat() const {
	...
	unsigned const dl_ndiv5(dlights ? max(1, ndiv/5) : 1);
	setup_draw_ship();
	setup_exp_texture();
	...
	draw_cylinder(1.0, 0.5, 0.5, ndiv, dl_ndiv5, 1); // body
	glTranslatef(0.0, 0.0, 1.0);
	color_b.do_glColor();
	draw_cylinder_nstacks(1.9, 0.11, 0.0, get_ndiv(ndiv/2), dl_ndiv5, 0); // point
...
void uobj_draw_data::draw_starbase() const {
	...
	int const cyl_nsta(dlights ? max(1, ndiv/4) : 1), spoke_nsta(dlights ? max(1, ndiv/6) : 1);
	...
	// draw center
	color_b.do_glColor();
	cobjs[1]->draw_cylin(cyl_ndiv, cyl_nsta, 0);

	// draw spokes
	set_cloak_color(BRONZE_C);

	for (unsigned i = 2; i < cobjs.size(); ++i) {
		cobjs[i]->draw_cylin(spoke_ndiv, spoke_nsta, 0);
	}
	end_exp_texture();
}
...
void uobj_draw_data::draw_bshuttle() const {
	...
	draw_ehousing_pair(1.0, 0.18, 0.18, 0.22, 1.6, 0.0); // length r1 r2 lcone dx dy
	glPopMatrix(); // undo invert_z()
...
void uobj_draw_data::draw_tractor() const { // could be better
	...
	setup_draw_ship();
	draw_cube(point(0.0, 0.0, -0.2), 1.6, 1.2, 2.0, 0, (dlights ? max(1, ndiv/3) : 1), 1);
	color_b.do_glColor();
	draw_cube(point(0.0, 0.0, 1.0), 0.4, 0.2, 0.4, 0, 1);

	for (unsigned i = 0; i < 2; ++i) {
		glPushMatrix();
		glTranslatef(-1.05, (i ? -0.2 : 0.2), -0.8);
		draw_ehousing_pair(1.0, 0.25, 0.25, 0.3, 2.1, 0.0); // length r1 r2 lcone dx dy
		glPopMatrix();
	}
	enable_blend();
	set_cloak_color(colorRGBA(1.0, 1.0, 1.0, 0.5));
	enable_blend();
	draw_cube(point(0.0, 0.0, 1.1), 0.8, 0.4, 0.6, 0, 1);
	glPopMatrix(); // undo invert_z()
	glPushMatrix();
	glTranslatef(0.0, 0.0, 0.4);

	for (unsigned i = 0; i < 2; ++i) {
		glTranslatef(0.0, (i ? 0.4 : -0.2), 0.0);
		draw_engine_pair(WHITE, (i << 1), 0.9, 1.0, 0.0, 0.6); // escale dx dy dz
	}
	glPopMatrix();
}
...
void uobj_draw_data::draw_gunship() const {
	...
	glPushMatrix();
	glScalef(1.0, 1.0, 2.0);
	draw_sphere_dlist(point(0.0, 0.0, 0.325), 0.25, 2*ndiv/3, 0);
	glPopMatrix();
	...
	gluCylinder(quadric, 0.45, 0.25,  1.6, ndiv2, (dlights ? ndiv4 : 1)); // main body
	...
	if (ndiv > 4) { // engine housings
		...
		for (unsigned i = 0; i < 4; ++i) {
			...
			gluCylinder(quadric, 0.05, 0.05, 0.25, ndiv2, (dlights ? ndiv4 : 1));
			glTranslatef(0.0, 0.0, 0.05);
			if (ndiv > 12) draw_circle_normal(0.0, 0.05, ndiv2, 0);
			glPopMatrix();
		}
		set_lighted_sides(1);
	}
...
void uobj_draw_data::draw_dwcarrier() const {
	...
	unsigned const ndiv8(get_ndiv(ndiv/8)), ndiv_dl((dlights ? ndiv8 : 1));
	setup_draw_ship();
	...
	if (phase1) {
		...
		for (unsigned i = 0; i < 4; ++i) { // draw body
			...
			if (ndiv < 10) { // approximate low-detail version with only one cylinder and no ends
				draw_cylinder_nstacks(2.4, 0.72, 0.15, ndiv35, ndiv_dl, 0);
			}
			else {
				glPushMatrix();
				...
				for (unsigned j = 0; j < 4; ++j) {
					...
					draw_cylinder(length, (0.72 - 0.2*j), (0.61 - 0.2*j), ndiv35, ndiv_dl, (ndiv >= 8), 0, 1);
		...
		if (ndiv > 4) { // engine housings
			...
			for (unsigned i = 0; i < 2; ++i) {
				glPushMatrix();
				glTranslatef(1.0*(1.0 - 2.0*i), 0.0, -1.35);
				draw_cylinder(1.0, 0.38, 0.1, ndiv2, ndiv_dl, (ndiv >= 8), 1, 0);
				glPopMatrix();
			}
			glPopMatrix();
		}
	...
	if (phase2 && ndiv > 6) { // razorback
		if (ndiv > 9) { // draw "holes" in backwards depth order
			...
			for (unsigned i = 0; i < 5; ++i) {
				glPushMatrix();
				glTranslatef(-0.01, (0.44 - 0.04*i), (-0.84 + 0.3*i));
...
void uobj_draw_data::draw_dwexterm() const {
	...
	unsigned const ndiv2(get_ndiv(ndiv/2)), ndiv3(get_ndiv(ndiv/3)), ndiv4(get_ndiv(ndiv/4)), ndiv4x(dlights ? ndiv4 : 1);
	setup_draw_ship();
	...
	draw_cube(point(0.0, 0.01,  0.45), 0.12, 0.14, 1.5, 1, ndiv4x, 1);
	draw_cube(point(0.0, 0.01, -1.15), 0.12, 0.14, 0.1, 1, 1);
	draw_cube(point(0.0, 0.00, -0.70), 0.36, 0.16, 0.8, 1, ndiv4x, 1);
	glDisable(GL_TEXTURE_2D);
	...
	for (unsigned i = 0; i < 6; ++i) { // "ribs"
		...
		glutSolidTorus(0.02, 0.25, get_ndiv(ndiv/5), get_ndiv(4*ndiv/5));
		glPopMatrix();
	}
	glPushMatrix();
	glTranslatef(0.0, -0.38, 0.01);
	glScalef(1.2, 0.8, 1.0);
	draw_cylinder(0.56, 0.028, 0.028, ndiv2, ndiv4x, 1);
	glPopMatrix();
	tex_color.do_glColor();
	set_ship_texture(BCUBE2_TEX);

	// forward deck
	draw_cube(point(0.0, 0.11, 0.30), 0.32, 0.06, 0.60, 1, ndiv4x, 1);
	...
	for (unsigned i = 0; i < 2; ++i) { // forward "wings"
		...
		draw_cylinder(0.62, 0.2, 0.2, 6, ndiv4x, 1);
		glPopMatrix();
	}

	// rear deck
	draw_cube(point(0.0, 0.11, -0.70), 0.40, 0.06, 0.80, 1, ndiv4x, 1);
	glPushMatrix();
	glTranslatef(0.0, 0.1, -1.2);
	glScalef(1.0, 0.13, 1.0);
	draw_cylinder(0.1, 0.1, 0.22, 6, ndiv4x, 1);
	glPopMatrix();
	...
	for (unsigned i = 0; i < 2; ++i) { // rear "wings"
		...
		draw_cylinder(0.78, 0.3, 0.1, 6, ndiv4x, 1);
		glPopMatrix();
	}
	...
	draw_cylinder(0.4, 0.0, 0.1, ndiv, ndiv4x, 1);
	glPopMatrix();
	...
	glScalef(1.0, 1.0, 1.8);
	glRotatef(-90.0, 1.0, 0.0, 0.0);
	draw_cylinder(0.08, 0.14, 0.06, ndiv, ndiv4x, 1);
	glPopMatrix();
	color_a.do_glColor();
	end_ship_texture();
	...
	if (ndiv > 4) {
		...
		for (unsigned i = 0; i < 4; ++i) { // engines
			...
			draw_ehousing_pair(0.6-0.08*i, 0.04, 0.0, 0.034, 0.48+2.0*val, 0.0);
			glPopMatrix();
		}
	}
	if (first_pass) {
...
void uobj_draw_data::draw_supply() const {
	...
	draw_cylinder(point(0.0, 0.0, 1.0), 0.4, 0.5, 0.5, ndiv, nstacks, 1); // front ring
	color_b.do_glColor();
	glPushMatrix();
	glTranslatef(0.0, 0.0, -1.0);
	glScalef(1.0, 1.8, 1.0);
	draw_cylinder_nstacks(1.8, 0.1, 0.1, ndiv2, (dlights ? max(1, ndiv/4) : 1), 0); // backbone
	glPopMatrix();
	glPushMatrix();
	glTranslatef(0.0, 0.0, 0.8);
	draw_cylinder_nstacks(0.8, 0.45, 0.45, ndiv, nstacks, 0); // front cylinder
	glTranslatef(0.0, 0.0, -0.6);
	draw_cylinder_nstacks(0.6, 0.00, 0.45, ndiv, nstacks, 0);
	glTranslatef(0.0, 0.0, -1.2);
	glScalef(1.0, 1.5, 1.0);
	draw_cylinder_nstacks(0.6, 0.3, 0.0, ndiv, (dlights ? max(1, ndiv/6) : 1), 0);
	glPopMatrix();
	glPushMatrix();
	glScalef(1.0, 1.0, 0.5);
	draw_sphere_dlist(point(0.0, 0.0, 3.2), 0.45, ndiv, 0, 1); // front sphere
	glPopMatrix();
	draw_cube(point(0.0, 0.0, -1.3), 0.6, 0.9, 0.6, 0, (dlights ? max(1, ndiv/4) : 1), 1); // rear

	if (ndiv > 4) { // draw engines
		for (unsigned i = 0; i < 3; ++i) {
			glPushMatrix();
			glTranslatef(-0.25, (-0.4 + 0.4*i), -1.7);
			draw_ehousing_pair(0.55, 0.2, 0.12, 0.1, 0.5, 0.0);
			glPopMatrix();
		}
	}
	glDisable(GL_LIGHTING);
...
void uobj_draw_data::draw_saucer(bool rotated, bool mothership) const {
	...
	if (ndiv > 4) {
		...
		if (mothership) {
			draw_cylinder_nstacks(0.6, 0.15, 0.0, ndiv2, nstacks, 0); // pointy top cylinder
		}
		else {
			glScalef(1.0, 1.0, 0.5);
	...
	if (ndiv > 6) {
		...
		for (unsigned i = 0; i < nwpts; ++i) {
			...
			point const pos(0.6*cosf(theta), 0.6*sinf(theta), 0.16);
			draw_sphere_dlist(pos, 0.065, ndiv2, 0);
		}
	}
	if (ndiv > 8) {
		...
		for (unsigned i = 0; i < nlights; ++i) {
			bool const is_lit(powered && ((i+(on_time>>2))&3) == 0);
			...
			point const pos(cosf(theta), sinf(theta), 0.0);
			draw_sphere_dlist(pos, 0.045, ndiv4, 0);
	...
	if (ndiv > 3) {
		...
		for (unsigned i = 0; i < 2; ++i) {
			...
			for (unsigned j = 0; j < 4; ++j) {
				...
				draw_cylin_fast(r, r, 0.32, ndiv4, 0, 0, 0);
			}
			glPopMatrix();
		}
	}
...
void uobj_draw_data::draw_colony(bool armed, bool hw, bool starport) const {
	...
	glPushMatrix();
	glTranslatef(0.0, 0.0, -1.0);
	draw_cylinder(1.5, 1.0, 1.0, 3*ndiv/2, (dlights ? max(1, ndiv/4) : 1), 1, 1, 0);
	glPopMatrix();

	glPushMatrix();
	glScalef(1.0, 1.0, 0.5);
	draw_sphere_dlist(point(0.0, 0.0, 1.0), 1.0, 3*ndiv/2, 0);
	glPopMatrix();
	color_b.do_glColor();
	...
	if (hw && ndiv > 3) {
		unsigned const nstacks(dlights ? max(1, ndiv/6) : 1);

		for (unsigned i = 0; i < 3; ++i) {
			float const theta(TWO_PI*i/3.0), x(1.05*cosf(theta)), y(1.05*sinf(theta));
			glPushMatrix();
			glTranslatef(x, y, -0.6);
			draw_cylinder(1.1, 0.2, 0.2, ndiv2, nstacks, 1, 1, 0);
			draw_sphere_dlist(point(0.0, 0.0, 0.0), 0.2, ndiv2, 0, 0);
			draw_sphere_dlist(point(0.0, 0.0, 1.1), 0.2, ndiv2, 0, 0);
			glPopMatrix();
		}
	}
	if (starport && ndiv > 3) {
