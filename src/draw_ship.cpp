...
void uobj_draw_data::draw_us_destroyer() const {
	...
	if (ndiv > 6) { // draw parallel beams
		glPushMatrix();
		glTranslatef(0.0, 0.0, 0.8);
		glScalef(0.6, 0.04, 0.9);
		glutSolidCube(1.8);
		glScalef(0.0667, 15.0, 1.0);
		glutSolidCube(1.8);
		glPopMatrix();
	}
...
void uobj_draw_data::draw_us_cruiser(bool heavy) const {
	...
	draw_cylinder_nstacks(1.8, 0.38, 0.26, ndiv, (dlights ? max(1, ndiv/5) : 1), textured);
	glPushMatrix();
	...
	for (unsigned i = 0; i < nengines; ++i) { // draw engine housings
		...
		if (ndiv > 8) { // draw engines
			...
			draw_cylin_fast(0.06, 0.08, 0.03, ndiv3, textured, 0);
			glPopMatrix();
		}
		set_lighted_sides(1);
	}
...
void uobj_draw_data::draw_armageddon(mesh2d const &surface_mesh) const {
	...
	color_b.do_glColor();
	draw_cube(point(0.0, -0.68, 0.0), 0.6, 0.4, 1.2, 1, (dlights ? max(1, ndiv/3) : 1), 1, 1.0, 1); // box
	end_ship_texture();
...
void uobj_draw_data::draw_borg(bool is_cube, bool is_small) const {
	...
	if (phase1) {
		...
		if (is_cube) {
			draw_cube(all_zeros, 1.95, 1.95, 1.95, 1, ndiv2, 0, 1.0, 0, &view_dir);
		}
		else {
	...
	if (phase2) {
		...
		if (is_cube) {
			draw_cube(all_zeros, 2.0, 2.0, 2.0, 1, ndiv2, 0, 1.0, 0, &view_dir);
		}
		else {
...
void uobj_draw_data::draw_dwexterm() const {
	...
	draw_cube(point(0.0, 0.01,  0.45), 0.12, 0.14, 1.5, 1, ndiv4x, 1, 2.0, 1);
	draw_cube(point(0.0, 0.01, -1.15), 0.12, 0.14, 0.1, 1, 1, 2.0, 1);
	draw_cube(point(0.0, 0.00, -0.70), 0.36, 0.16, 0.8, 1, ndiv4x, 1, 2.0, 1);
	glDisable(GL_TEXTURE_2D);
	...
	for (unsigned i = 0; i < 6; ++i) { // "ribs"
		...
		glutSolidTorus(0.02, 0.25, ndiv4, get_ndiv(ndiv));
		glPopMatrix();
	}
	glPushMatrix();
	...
	// forward deck
	draw_cube(point(0.0, 0.11, 0.30), 0.32, 0.06, 0.60, 1, ndiv4x, 1, 1.0, 1);
	...
	// rear deck
	draw_cube(point(0.0, 0.11, -0.70), 0.40, 0.06, 0.80, 1, ndiv4x, 1, 1.0, 1);
	glPushMatrix();
...
void uobj_draw_data::draw_supply() const {
	...
	draw_cylinder_nstacks(1.8, 0.1, 0.1, ndiv2, (dlights ? max(1, ndiv/4) : 1), textured); // backbone
	glPopMatrix();
	glPushMatrix();
	glTranslatef(0.0, 0.0, 0.8);
	draw_cylinder_nstacks(0.8, 0.45, 0.45, ndiv, nstacks, textured); // front cylinder
	glTranslatef(0.0, 0.0, -0.6);
	draw_cylinder_nstacks(0.6, 0.00, 0.45, ndiv, nstacks, textured);
	glTranslatef(0.0, 0.0, -1.2);
	glScalef(1.0, 1.5, 1.0);
	draw_cylinder_nstacks(0.6, 0.3, 0.0, ndiv, (dlights ? max(1, ndiv/6) : 1), textured);
	glPopMatrix();
	glPushMatrix();
	glScalef(1.0, 1.0, 0.5);
