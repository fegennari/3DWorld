...
void building_t::toggle_light_object(room_object_t const &light, point const &sound_pos) { // called by the player
	...
	bool const in_basement(light.z1() < ground_floor_z1), in_closet(light.in_closet());
	float const floor_spacing(get_window_vspace());
	auto objs_end(interior->room_geom->get_placed_objs_end()); // skip buttons/stairs/elevators
	bool updated(0), is_lamp(1), is_first_light(1), is_lit(0);
	...
	for (auto i = interior->room_geom->objs.begin(); i != objs_end; ++i) { // toggle all lights on this floor of this room
		...
		if (in_closet != i->in_closet()) continue; // closet + room light are toggled independently
		...
		else if (!in_closet) { // no object state change for a broken light
...
breaker_zone_t building_interior_t::get_circuit_breaker_info(unsigned zone_id, unsigned num_zones, float floor_spacing) const {
	...
	if (!elevators.empty()) { // elevators are always zone 0 (lower left or right breaker)
		if (zone_id == 0) return breaker_zone_t(RTYPE_ELEVATOR, 0, 0, -1, 0);
	...
	bool const is_dup(zone_id >= num_rooms);
	...
	return breaker_zone_t(ret_rtype, room_start, room_end, pri_room, is_dup);
}
void building_t::toggle_circuit_breaker(bool is_on, unsigned zone_id, unsigned num_zones) {
	...
	if (zone.invalid()) return; // no rooms for this zone; allow duplicates so that all breakers can have an effect
	...
	if (ball.taken_level == 1) { // moving toward a pocket
		for (unsigned p = 0; p < 6; ++p) {
			...
			if (dist_less_than(old_pos, center, move_dist)) { // reached the center of the hole
				new_pos = center;
				ball.taken_level = 2;
				gen_sound_thread_safe(SOUND_KICK_BALL, local_to_camera_space(new_pos), 1.0, 2.5);
			}
		...
		if (ball.z2() < play_area.z1()) { // fell below the surface of the table, remove it
			gen_sound_thread_safe(SOUND_KICK_BALL, local_to_camera_space(old_pos), 0.5, 2.5);
			obj_dynamic_to_static(ball, *interior);
			ball.remove();
		}
	...
	for (unsigned n = 0; n < num_steps; ++n) {
		...
		for (unsigned p = 0; p < 6; ++p) {
			...
			if (pocket.contains_cube_xy(ball_bc)) {
				...
				pool_ball_in_pocket(ball.item_flags);
			}
			play_area.expand_by_xy(radius); // expand play area to allow the ball to partially intersect the edges
			break;
		} // for p
...
void building_t::update_player_interact_objects(point const &player_pos) { // Note: player_pos is in building space
	...
	for (auto c = objs.begin(); c != objs.end(); ++c) {
		...
		if (c->type == TYPE_THEFT_SENS) {
			...
			if (was_room_stolen_from(c->room_id)) {
				cube_t sensor_area(*c);
				sensor_area.d[c->dim][!c->dir] += (c->dir ? -1.0 : 1.0)*2.0*c->dz();
				sensor_area.z2() = sensor_area.z1() + floor_spacing;
				make_active = sensor_area.contains_pt(camera_rot);
			}
			if (make_active) { // raise the alarm
	...
	if (player_in_this_building) { // interactions only run for player building
		...
		if (interior->room_geom->fire_manager.get_closest_fire(camera_rot, player_radius, player_z1, player_z2, &fire_pos)) {
			player_take_damage(0.006); // small amount of fire damage
			dist_to_fire_sq  = p2p_dist(camera_rot, fire_pos);
		...
		if (!player_room_no_power && player_room_ix >= 0 /*&& !is_house*/) { // check for sounds; should this be for office buildings only?
			...
			else if (room_type == RTYPE_SERVER && !is_datacenter()) {hum_amt += 0.20; hum_freq = 120.0;}
...
bool is_blocking_obj_on_top_surface(room_object_t const &o) { // objects on tables, counters, desks, etc.
	return (o.type == TYPE_PLANT || o.type == TYPE_KEYBOARD || o.is_a_drink() || o.type == TYPE_MWAVE || is_ball_type(o.type) || o.type == TYPE_PLATE ||
		...
		o.type == TYPE_CUP || o.type == TYPE_TOASTER || o.type == TYPE_SILVER || o.type == TYPE_PIZZA_BOX || o.type == TYPE_PLANT_MODEL || o.type == TYPE_CONF_PHONE ||
		o.type == TYPE_HARDHAT || o.type == TYPE_TOPHAT || o.type == TYPE_COMP_MOUSE || o.type == TYPE_VIS_PHONE || o.type == TYPE_HANDGUN || o.type == TYPE_JAR ||
		o.type == TYPE_FOOD_TUB || o.type == TYPE_MILK || o.type == TYPE_LAVALAMP || o.type == TYPE_FOOD_TRAY || o.type == TYPE_CARD_DECK || o.type == TYPE_CASHREG ||
		o.type == TYPE_APPLE || o.type == TYPE_BANANA || o.type == TYPE_CANDLE || o.type == TYPE_FLASHLIGHT);
}
