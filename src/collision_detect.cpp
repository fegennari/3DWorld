...
class set_true_obj_height_mgr : public cobj_query_callback {

	point &pos;
	point const &lpos;
	float &zvel;
	int type, id, any_coll, moved, xpos, ypos;
	bool skip_dynamic, is_camera, is_player;
	float radius, z1, z2, step, zceil, zfloor, mh, zmu;

public:
	set_true_obj_height_mgr(point &pos_, point const &lpos_, float step_height, float &zvel_, int type_, int id_, bool sd)
		: pos(pos_), lpos(lpos_), zvel(zvel_), type(type_), id(id_), any_coll(0), moved(0), skip_dynamic(sd),
		is_camera(type == CAMERA), is_player(is_camera || (type == SMILEY && id >= 0)), radius(object_types[type].radius),
		z1(pos.z - radius), z2(pos.z + radius), step(step_height*radius), zceil(0.0), zfloor(0.0)
	{
		if (is_player) assert(id >= CAMERA_ID && id < num_smileys);
		xpos = get_xpos(pos.x) - xoff;
		ypos = get_ypos(pos.y) - yoff;
	}

	int init(bool test_only) {
		if (point_outside_mesh(xpos, ypos)) {
		...
		mh = zmu = int_mesh_zval_pt_off(pos, 1, 0); // *** step height determined by fticks? ***
	...
	// return value: 0 = no change, 1 = moved up, 2 = falling (unused), 3 = stuck
	int finalize(bool flight, bool on_snow) {
		for (unsigned d = 0; d < 1+unsigned(skip_dynamic != 0); ++d) {
			get_coll_line_cobjs_tree(point(pos.x, pos.y, z2), point(pos.x, pos.y, czmin), -1, NULL, this, (d != 0), 0);
		}
		if (moved == 3) return moved; // stuck
	...
	virtual void register_cobj(coll_obj const &cobj) {
		if (moved == 3)                return;
		if (cobj.d[2][0] > z2)         return; // above the top of the object - can't affect it
		if (!cobj.contains_pt_xy(pos)) return; // test bounding cube
		if (cobj.no_collision())       return;
		if (skip_dynamic && cobj.status == COLL_DYNAMIC) return;
		float zt, zb;
		int coll(0);
...
{
#if 0
	set_true_obj_height_mgr ohm(pos, lpos, step_height, zvel, type, id, skip_dynamic);
	if (!ohm.init(test_only)) return 0;
	return ohm.finalize(flight, on_snow);
#endif
