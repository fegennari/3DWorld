...
struct divider_t : public oriented_city_obj_t {
	...
	bool ends_clipped=0, use_dir_for_birds=0;
	vect_cube_t ivy_avoid; // for house walls

	divider_t() {}
	divider_t(cube_t const &c, unsigned type_, bool dim_, bool dir_, bool ec, unsigned sd, unsigned dix, unsigned pix, unsigned cix, unsigned sdir=0, bool udb=0) :
		oriented_city_obj_t(c.get_cube_center(), c.get_bsphere_radius(), dim_, dir_), type(type_),
		divider_ix(dix), plot_ix(pix), city_ix(cix), skip_dims(sd), street_dir(sdir), ends_clipped(ec), use_dir_for_birds(udb) {bcube = c;}
	static void pre_draw (draw_state_t &dstate, bool shadow_only);
	static void post_draw(draw_state_t &dstate, bool shadow_only);
	void draw(draw_state_t &dstate, city_draw_qbds_t &qbds, float dist_scale, bool shadow_only) const;
	bool proc_sphere_coll(point &pos_, point const &p_last, float radius_, point const &xlate, vector3d *cnorm) const;
...
struct swimming_pool_t : public oriented_city_obj_t { // Note: dim and dir are used for the ladder and face toward the house
	...
	float get_radius      () const {assert(above_ground); return 0.5f*min(bcube.dx(), bcube.dy());}
...
struct bb_hoop_t : public model_city_obj_t {
	bb_hoop_t(point const &pos_, float height, bool dim_, bool dir_) : model_city_obj_t(pos_, height, dim_, dir_, get_model_id()) {}
	virtual unsigned get_model_id() const {return OBJ_MODEL_BB_HOOP;}
};
...
struct sculpture_t : public city_obj_t {
	int rseed;
	colorRGBA color;
...
struct ww_elevator_t : public oriented_city_obj_t { // elevator connected to overhead walkway
	...
	unsigned get_num_floors  () const {return round_fp((bcube.z2() - ww_bcube.z1())/floor_spacing);} // in walkway
...
struct city_bldg_t : public obj_with_roof_pavement_lights_t, public reservable_t {
	...
	vect_cube_with_ix_t barrels; // ix stores material
...
struct wind_turbine_t : public model_city_obj_t {
	...
	bool proc_sphere_coll(point &pos_, point const &p_last, float radius_, point const &xlate, vector3d *cnorm) const;
	void draw(road_draw_state_t &dstate, float fog_dist, bool shadow_only) const;
...
struct park_path_t : public city_obj_t {
	...
	colorRGBA color;
	cube_t plot;
	bool dim, is_creek; // else sidewalk
	bool has_creek_crossing=0; // for sidewalks

	park_path_t(float hwidth_, colorRGBA const &color_, cube_t const &plot_, bool dim_, bool is_creek_) :
		hwidth(hwidth_), color(color_), plot(plot_), dim(dim_), is_creek(is_creek_) {}
...
struct bird_place_t {
	...
	bird_place_t(point const &pos_, bool dim, bool dir, bool use_orient_=1) : pos(pos_), orient(vector_from_dim_dir(dim, dir)), on_ground(0), use_orient(use_orient_) {}
...
public:
	...
	void next_frame(float fticks_stable);
...
class cubes_grid_t {
	static constexpr unsigned NDIV=8; // in each dim
...
private:
	...
	template<typename T> void draw_objects(vector<T> const &objs, city_obj_groups_t const &groups, draw_state_t &dstate, float dist_scale,
		bool shadow_only, bool has_immediate_draw=0, bool draw_qbd_as_quads=0, float specular=0.75, float shininess=50.0, float metalness=0.0);
