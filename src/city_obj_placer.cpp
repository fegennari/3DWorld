	...
	if (is_park) {
		...
		if (1) { // try to place pond(s)
			...
			for (unsigned n = 0; n < 200; ++n) { // 200 tries to place a pond
				...
				gen_xy_pos_in_cube(center, pond_area, rgen);
				cube_t pond(center);
		...
		if (building_obj_model_loader.is_model_valid(OBJ_MODEL_PICNIC)) {
			...
			for (unsigned n = 0; n < city_params.max_benches_per_plot; ++n) { // use the same max count as benches
				...
				if (building_obj_model_loader.is_model_valid(OBJ_MODEL_BICYCLE) && rgen.rand_float() < 0.35) {
					...
					bicycle_t const bike(pos, bike_height, !dim, rgen.rand_bool()); // random dir
		...
		for (auto p = ppaths.begin()+paths_start; p != ppaths.end(); ++p, ++pix) {
	...
	if (!is_park && building_obj_model_loader.is_model_valid(OBJ_MODEL_FHYDRANT)) {
		...
		for (unsigned dim = 0; dim < 2; ++dim) {
			...
			for (unsigned dir = 0; dir < 2; ++dir) {
				...
				vector3d const orient(vector_from_dim_dir(!dim, dir)); // oriented perpendicular to the road
	...
	unsigned city_id, unsigned plot_id_offset, bool have_cars, bool is_residential, bool have_streetlights)
{
	...
	sculpt_groups  .create_groups(sculptures,all_objs_bcube);
...
void city_obj_placer_t::get_ponds_in_xy_range(cube_t const &range, vect_cube_t &pond_bcs, bool for_plants) const {
	if (!range.intersects_xy(pond_groups.get_bcube())) return;

	for (pond_t const &p : ponds) {
		if (for_plants && !p.cat_tails.empty()) continue; // no extra plants if there are already cat tails
