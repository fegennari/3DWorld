...
void city_obj_placer_t::clear() {
	...
	stopsigns.clear(); flags.clear(); ppaths.clear(); swings.clear(); bikes.clear(); plants.clear();
	...
	flag_groups.clear(); nrack_groups.clear(); ppath_groups.clear(); swing_groups.clear(); bike_groups.clear(); plant_groups.clear();
	...
	vector<road_t> const &roads, vector<road_isec_t> isecs[3], unsigned city_id, bool have_cars, bool is_residential, bool have_streetlights)
{
	...
	if (0) { // debug info printing
		cout << TXT(benches.size()) << TXT(planters.size()) << TXT(trashcans.size()) << TXT(fhydrants.size()) << TXT(sstations.size()) << TXT(fountains.size())
			 ...
			 << TXT(newsracks.size()) << TXT(ppaths.size()) << TXT(swings.size()) << TXT(bikes.size()) << TXT(plants.size()) << endl;
	}
	if (add_parking_lots) {
...
bool city_obj_placer_t::line_intersect(point const &p1, point const &p2, float &t) const { // p1/p2 in world space; uses object bcubes
	...
	// mboxes, swings, bikes, and plants are ignored because they're small or not simple shapes
	return ret;
}
...
bool city_obj_placer_t::get_color_at_xy(point const &pos, colorRGBA &color, bool skip_in_road) const {
	...
	// Note: ppoles, hcaps, manholes, mboxes, tcones, pladders, signs, stopsigns, flags, pigeons, birds, swings, bikes, and plants are skipped for now
	return 0;
}
