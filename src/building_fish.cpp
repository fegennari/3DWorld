...
class fish_pond_t : public fish_cont_t {
public:
...
class fish_manager_city_t {
	map<unsigned, fish_pond_t> ponds;
	vector<unsigned> ponds_visible;
	float anim_time=0.0;
public:
	bool any_visible() const {return !ponds_visible.empty();}

	void pond_visible(cube_t const &pond, unsigned pond_id) {
		if (ponds.find(pond_id) == ponds.end()) {ponds.insert(make_pair(pond_id, fish_pond_t(pond, pond_id)));}
		ponds_visible.push_back(pond_id);
	}
	void next_frame() {
		for (unsigned id : ponds_visible) { // only need to update fish in visible ponds
			auto it(ponds.find(id));
			assert(it != ponds.end());
	...
	void draw_fish(shader_t &s) { // non-const, modifies ponds_visible
		if (!any_visible()) return;
		sort_and_unique(ponds_visible); // not needed?
		animation_state_t anim_state(1, ANIM_ID_FISH_TAIL); // enabled=1; animation_scale and model_delta_height are not set or used

		for (unsigned id : ponds_visible) {
			auto it(ponds.find(id));
			assert(it != ponds.end());
			it->second.draw(s, anim_state, anim_time);
		}
		anim_state.clear_animation_id(s); // clear animations
		check_mvm_update();
	}
...
fish_manager_bldg_t fish_manager_bldg;
fish_manager_city_t fish_manager_city;

void register_fishtank(room_object_t const &obj, bool is_visible) {fish_manager_bldg.register_fishtank(obj, is_visible);}

void end_fish_draw(shader_t &s, bool inc_pools_and_fb) {
	fish_manager_bldg.end_objs();
	fish_manager_bldg.draw_fish(s, inc_pools_and_fb);
}

void register_fish_pond_visible(cube_t const &pond, unsigned pond_id) {fish_manager_city.pond_visible(pond, pond_id);}
void next_fish_frame() {fish_manager_city.next_frame();}
void draw_fish_in_ponds(shader_t &s) {fish_manager_city.draw_fish(s);}


bool building_t::begin_fish_draw() const { // returns true of pool or basement water is visible
	assert(interior);
	fish_manager_bldg.next_frame(*this);
	...
	if (has_pool()) {
		if (!fish_manager_bldg.has_swimming_pool()) { // not yet setup
			...
			fish_manager_bldg.register_swimming_pool(interior->pool, pool_ramp, water_bcube, obstacles, rseed);
		}
	}
		...
		if (!fish_manager_bldg.has_flooded_basement()) { // not yet setup
			...
			fish_manager_bldg.register_flooded_basement(water_bcube, get_window_vspace(), obstacles, rseed);
		}
		fish_manager_bldg.register_doors_state(interior->door_stacks, interior->doors);
	}
	return 1;
}
