...
void cobj_triangle_visitor::proc_poly(point const *p, unsigned npts) {
	...
	if (npts == 3) {proc_tri(p);}
	else if (npts == 4) {proc_quad(p);}
	else {assert(0);}
}

template<typename V> void cobj_triangle_visitor::proc_triangle_verts(vector<V> const &verts) {

	assert(!(verts.size()%3)); // must all be triangles

	for (unsigned i = 0; i < verts.size(); i += 3) {
		point const pts[3] = {verts[i].v, verts[i+1].v, verts[i+2].v};
		proc_tri(pts);
	}
}
...
void cobj_triangle_visitor::proc_cobj(coll_obj const &c) {
	...
	if (c.is_thin_poly()) {proc_poly(c.points, c.npoints); return;} // special case optimization for polygons
	verts.clear();
	c.get_shadow_triangle_verts(verts, N_SPHERE_DIV);
	proc_triangle_verts(verts);
}
