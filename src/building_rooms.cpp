...
void building_t::place_plant_on_obj(rand_gen_t rgen, room_object_t const &place_on, unsigned room_id, float tot_light_amt, cube_t const &avoid) {
	...
	plant.set_from_sphere(center, radius);
...
void building_t::add_bathroom_windows(room_t const &room, float zval, unsigned room_id, float tot_light_amt) {
	...
	for (unsigned dim = 0; dim < 2; ++dim) {
		for (unsigned dir = 0; dir < 2; ++dir) {
			...
			objs.emplace_back(c, TYPE_WINDOW, room_id, dim, dir, RO_FLAG_NOCOLL, max(tot_light_amt, 1.0f), SHAPE_CUBE, WHITE); // always lit
		} // for dir
	} // for dim
}
			...
			for (unsigned dir = 0; dir < 2; ++dir) {
				...
				for (unsigned f = 0; f < num_floors; ++f, z += window_vspacing) {
					...
					for (auto c = trim_cubes.begin(); c != trim_cubes.end(); ++c) {
						...
						set_cube_zvals(ceil_trim, ceil_trim_z1, ceil_trim_z2);
						objs.emplace_back(ceil_trim, TYPE_WALL_TRIM, 0, dim, !dir, flags, 1.0, SHAPE_ANGLED, trim_color); // ceiling trim
					}
				} // for f
			} // for dir
