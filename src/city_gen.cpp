...
void city_shader_setup(shader_t &s, cube_t const &lights_bcube, bool use_dlights, int use_smap, int use_bmap, float min_alpha, bool force_tsl, float pcf_scale) {

	use_dlights &= (!lights_bcube.is_zero_area() && !dl_sources.empty());
	setup_smoke_shaders(s, min_alpha, 0, 0, 0, 1, use_dlights, 0, 0, ((use_smap == 1) ? 2 : 0), use_bmap, 0, use_dlights, force_tsl, 0.0, 0.0, 0, 0, 1); // is_outside=1
