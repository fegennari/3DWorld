...
public:
	...
	void gen_parking_lots_and_place_objects(vector<car_t> &cars, bool have_cars, bool &have_plot_dividers) {
		...
		city_obj_placer.gen_parking_and_place_objects(plots, plot_colliders, cars, roads, isecs, bcube, plot_cuts, city_id, plot_id_offset,
			have_cars, is_residential, !streetlights.empty());
	...
	void get_paths_and_creeks(cube_t const &region, vector<park_path_t const *> &paths) const {
		if (region.intersects_xy(bcube)) {city_obj_placer.get_paths_and_creeks(region, paths);}
	}
	...
	bool is_invalid_placement_for_cube(cube_t const &c) const { // Note: c is in global space
		...
		for (unsigned plot_ix = 0; plot_ix < plots.size(); ++plot_ix) {
			...
			if (plots[plot_ix].is_park) return 1; // don't place in parks; never gets here in the default world
...
private:
	...
	bool run_car_in_driveway_logic(car_t &car, vector<car_t> const &cars, rand_gen_t &rgen) const {
		...
		if (car.dest_gstation >= 0) { // gas station
		...
		return car_leave_driveway(car, cars, driveway, rgen); // else leaving driveway
	}
	bool car_leave_driveway(car_t &car, vector<car_t> const &cars, driveway_t const &driveway, rand_gen_t &rgen) const {
...
private:
	...
	bool select_avail_driveway_or_parking_space(car_t &car, rand_gen_t &rgen) const {
		...
		for (unsigned n = 0; n < 10; ++n) { // make 10 attempts to find a valid driveway
			...
			find_isec_to_enter_driveway(car, driveway);
			return 1;
		} // for n
		return 0; // failed
	}
	bool select_avail_gas_station_lane(car_t &car, rand_gen_t &rgen) const {
		...
		car.need_gas      = 1; // flag so that we know we haven't stopped yet
...
public:
	...
	cube_t get_road_bcube_for_car(car_base_t const &car) const {
		if (car.cur_road_type == TYPE_DRIVEWAY) {return get_driveway(car.cur_seg);} // is this case used? what about gas stations?
...
public:
	...
	void get_paths_and_creeks(cube_t const &region, vector<park_path_t const *> &paths) const {
		for (auto r = road_networks.begin(); r != road_networks.end(); ++r) {r->get_paths_and_creeks(region, paths);}
	}
	...
	bool update_car_dest(car_t &car) const {
		...
		if (car.cur_road_type == TYPE_DRIVEWAY ) return 0; // don't choose a new dest until we're out of the driveway
...
public:
	...
	void get_paths_and_creeks(cube_t const &region, vector<park_path_t const *> &paths) const {road_gen.get_paths_and_creeks(region, paths);}
...
void get_city_paths_and_creeks(cube_t const &region, vector<park_path_t const *> &paths) {city_gen.get_paths_and_creeks(region, paths);}
