...
void rgeom_mat_t::draw_geom() const {
	...
	draw_indexed_tri_verts(num_verts, num_ixs, GL_TRIANGLES);
}
...
void building_room_geom_t::invalidate_draw_data_for_obj(room_object_t const &obj, bool was_taken) {
	...
	if (obj.type == TYPE_LIGHT)    {invalidate_lights_geom();}
	...
	tid_nm_pair_t const &wall_tex, bool inc_text, float floor_ceil_gap, bldg_industrial_info_t const *ind_info)
{
	...
	for (unsigned i = six; i < eix; ++i) { // Note: iterating with indices to avoid invalid ref when add_nested_objs_to_verts() is called
		...
		case TYPE_FISHTANK:  add_fishtank_plants(c); break;
		...
		case TYPE_ROCK_WALL:  add_rock_wall  (c, tscale); break;
...
void building_room_geom_t::create_dynamic_vbos(building_t const &building, point const &camera_bs, vector3d const &xlate, bool play_clock_tick) {
	...
	for (elevator_t const &e : building.interior->elevators) {
		...
		int const floor_offset(building.calc_floor_offset(e.z1(), e_floor_spacing));
...
void add_handle_models_for_door(door_t const &d, door_rotation_t const &drot, bool residential, bool is_exterior,
	bool opens_inward, bool is_odoor, vector<door_handle_t> &door_handles)
{
	...
	handle_center.z += (is_exterior ? ((d.open ? -0.08 : -0.2) + (is_odoor ? -0.33 : 0.0)) : (residential ? 0.15 : -0.18))*handle_height;
...
void building_room_geom_t::create_door_vbos(building_t const &building) {
	...
	for (door_t const &d : building.interior->doors) { // interior doors; opens_out=0, exterior=0
		...
		else if (use_model) {add_handle_models_for_door(d, drot, residential, 0, 0, 0, door_handles);} // add model to door_handles; is_exterior=0, opens_inward=0, is_odoor=0
...
void building_room_geom_t::update_exterior_door_vbos(building_t const &building) {
	...
	for (unsigned dix = 0; dix < building.doors.size(); ++dix) { // add exterior door handles for house and restroom doors
		...
		if (d.type != tquad_with_ix_t::TYPE_HDOOR && !is_odoor) continue; // skip garage doors
		...
		if (open) {
			float const rot_sign(opens_inward ? 1.0 : -1.0);
			drot.angle = 90.0*rot_sign;
			drot.shift = 0.05*(dir ? -1.0 : 1.0)*rot_sign*door.get_width();
		}
		add_handle_models_for_door(door, drot, building.is_residential(), 1, opens_inward, is_odoor, door_handles); // is_exterior=1
	} // for d
}
...
void building_room_geom_t::draw_interactive_player_obj(carried_item_t const &c, shader_t &s, vector3d const &xlate) { // held by the player
	...
	if (c.type == TYPE_SPRAYCAN || c.type == TYPE_MARKER) {
		...
		rotate_verts(mat.itri_verts, plus_z, z_rot_angle, obj_center, 0); // rotate all itri verts about Z axis
	...
	vector3d const &xlate, unsigned building_ix, bool shadow_only, bool reflection_pass, unsigned inc_small, bool player_in_building, bool mall_visible)
{
	...
	bool const player_in_doorway(building.point_near_ext_door(camera_bs));
...
#pragma omp parallel num_threads(2)
			...
			if (!create_text) { // if drawers are opened/closed in the shadow pass, and there is a book, must create text as well before pending_objs are cleared
				for (room_object_t const &o : pending_objs) {
					if (o.type == TYPE_BOOK) {create_text = 1; break;}
				}
			}
			if (create_text) {
				create_text_vbos(skip_mall_objs);
