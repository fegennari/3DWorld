...
void voxel_model::render(unsigned lod_level, bool is_shadow_pass) { // not const because of vbo caching, etc.
	...
	if (is_shadow_pass) {
		...
		bool const use_bmap(params.detail_normal_map && (display_mode & 0x08) != 0 && (light_valid_and_enabled(0, lpos) || light_valid_and_enabled(1, lpos))); // || dlight_bump_map_enabled
