...
float get_merged_risers_radius(vect_riser_pos_t const &risers, unsigned pipe_type) {
	float radius[2] = {}; // flow_dir={0, 1}
	
	for (riser_pos_t const &p : risers) {
		radius[p.flow_dir] = get_merged_pipe_radius(radius[p.flow_dir], + p.radius, 4.0); // higher exponent to avoid pipes that are too large
	}
	if (pipe_type == PIPE_TYPE_HW && radius[0] > 0.0) {return radius[0];} // if there's a hot water heater, the main pipe matches its/their pipe radius
	if (radius[0] > 0.0 && radius[1] > 0.0) {return min(radius[0], radius[1]);} // closed loop with flow in both dirs: pipe radius is determined by min of in/out radius
	return max(radius[0], radius[1]); // single direction
}
...
struct pipe_t {
	...
	bool connected=0, conn_dir=0, outside_pg=0, for_extb=0, flow_dir=0, is_wh=0;

	pipe_t(point const &p1_, point const &p2_, float radius_, unsigned dim_, unsigned type_, unsigned end_flags_, bool for_extb_=0, bool fdir=0, bool wh=0) :
		p1(p1_), p2(p2_), radius(radius_), dim(dim_), type(type_), end_flags(end_flags_), connected(type != PIPE_RISER), for_extb(for_extb_), flow_dir(fdir), is_wh(wh) {}
	...
	unsigned room_id, unsigned num_floors, unsigned objs_start, float ceil_zval, rand_gen_t &rgen, unsigned pipe_type, bool allow_place_fail, unsigned depth, bool swap_dim)
{
	...
	for (riser_pos_t const &p : risers) {
		...
		pipes.emplace_back(point(pos.x, pos.y, pipe_zval), pos, p.radius, 2, PIPE_RISER, 0, p.in_extb, p.flow_dir, is_wh); // neither end capped
	...
	for (auto p = pipes.begin(); p != pipes.end(); ++p) {
		...
		float &v(p->p1[dim]);
		if (p->for_extb || p->is_wh) {xy_map[v].push_back(pipe_ix); continue;} // add without merging
		...
		for (unsigned ix : v.second) {
			...
			if (!pipe.for_extb && fabs(val - centerline) < r_main) {pipe.p1[d] = pipe.p2[d] = centerline;} // shift to connect directly to main pipe since it's close enough
				...
				if (skip) { // blocked, can't connect
					...
					pipe.is_wh = 0; // if shifted, no longer connects directly to the water heater
				}
				if (skip) {
	...
	for (pipe_t &p : pipes) {
		...
		if (p.type == PIPE_RISER) { // not for vertical drain pipes, since they're so short and mostly hidden above the connector pipes
			if (is_house && p.is_wh) {is_wh_riser = 1;} // water heater connections do get (special) fittings
			else continue;
		}
		...
		for (unsigned d = 0; d < 2; ++d) {
			if (is_wh_riser && d == 2) continue; // no top fitting
			...
			if (is_wh_riser) {pf.flags |= RO_FLAG_HANGING;} // flat ends
			...
			if (is_wh_riser) {set_wall_width(pf, pipe_zval, fitting_len_ext, 2);} // expand above and below horizontal pipe connection point
			else {pf.d[p.dim][!d] = pf.d[p.dim][d] + (d ? -1.0 : 1.0)*fitting_len_ext;} // extend in one dir
...
void building_t::add_extb_steam_pipes(rand_gen_t rgen) {
	//if (is_house) return;
	assert(has_ext_basement());
	unsigned first_room_ix(interior->ext_basement_hallway_room_id);
	assert(first_room_ix < interior->rooms.size());
	vect_room_object_t &objs(interior->room_geom->objs);
	// find first extended basement room object; we have to iterate over them all for each steam pipe to avoid lights, pipes, and other objects
	// because we dont' have a mapping from room to objects
	unsigned const objs_end(objs.size());
	unsigned objs_start(0);
	while (objs_start < objs.size() && objs[objs_start].room_id < first_room_ix) {++objs_start;}
	vect_cube_with_ix_t steam_pipes;
	unsigned const pipe_mat_ix(rgen.rand_bool()); // must be consistent for building so that connected pipes look correct
	for (unsigned r = first_room_ix; r < interior->rooms.size(); ++r) {add_hallway_steam_pipes(rgen, r, objs_start, objs_end, pipe_mat_ix, steam_pipes);}
}

void building_t::add_hallway_steam_pipes(rand_gen_t &rgen, unsigned room_id, unsigned objs_start, unsigned objs_end, unsigned pipe_mat_ix, vect_cube_with_ix_t &steam_pipes) {
	...
	for (unsigned d = 0; d < 2 && !added_pipe; ++d) { // side of hallway; only need to add to one side
		...
		if (is_obj_placement_blocked(pipe, room, 1, 1) || overlaps_other_room_obj(pipe, objs_start, 1, &objs_end)) continue; // bad placement
		float const pipe_centerline(pipe.get_center_dim(!dim));
		cube_t const pipe_base(pipe); // part of pipe inside this room
		bool cont_mask[2] = {0,0}; // continues into wall at each end
		bool ext_mask [2] = {0,0}; // exits into adjacent room at each end
		bool end_mask [2] = {0,0}; // ends (at a machine)

		// extend ends into nearby rooms if possible
		for (unsigned e = 0; e < 2; ++e) {
			point test_pt(0.0, 0.0, pipe_zval);
			test_pt[!dim] = pipe_centerline;
			...
			bool const is_machine_room(room2.get_room_type(0) == RTYPE_MACHINE);
			if (!room2.is_hallway && !is_machine_room) continue; // not hallway and not machine room: no extension
			...
			if (is_machine_room) { // find a nearby machine or boiler to connect to
				...
				for (cube_with_ix_t const &p : steam_pipes) {
				...
				if (!no_end_bracket) { // see if we can connect to a previous steam pipe crossing this room in the same dim
					for (cube_with_ix_t &p : steam_pipes) {
						bool const dim2(p.dx() < p.dy());
						if (dim2 != dim || p.z1() != pipe_ext.z1()) continue; // wrong dim or height
						...
						conn_pipe.d[!dim][1] = max(cent1, cent2);
						bool connected(0);
			...
			for (unsigned f = 0; f < 2; ++f) {
				...
				objs.emplace_back(bracket, TYPE_PIPE, room_id, dim, 0, bracket_flags, light_amt, SHAPE_CYLIN, color);
				objs.back().obj_id = bracket_mat_ix;
			} // for f
		} // for e
		...
		for (unsigned e = 0; e < 2; ++e) {
			...
			unsigned const end_flags(zdir ? RO_FLAG_ADJ_BOT : RO_FLAG_ADJ_TOP); // rounded bend at one end
			pipe.d[dim][e] = pipe_end;
			objs.emplace_back(v_pipe, TYPE_PIPE, room_id, 0, 1, (flags | end_flags), light_amt, SHAPE_CYLIN, color); // vertical
			objs.back().obj_id = pipe_mat_ix;
			...
			if (zdir == 0) { // add fitting to bottom (not enough space at top)
				...
				objs.emplace_back(bracket, TYPE_PIPE, room_id, 0, 1, (bracket_flags & ~RO_FLAG_ADJ_LO), light_amt, SHAPE_CYLIN, color); // vertical; skip bottom edges
				objs.back().obj_id = bracket_mat_ix;
			}
		} // for e
		unsigned const obj_id(objs.size()); // main pipe obj_id for steam emitter and connecting pipes
		objs.emplace_back(pipe, TYPE_PIPE, room_id, dim, 0, flags, light_amt, SHAPE_CYLIN, color);
		objs.back().obj_id = pipe_mat_ix;
		steam_pipes.emplace_back(pipe, obj_id);
		added_pipe = 1;
		...
		if (num_brackets > 0) { // likely true
			...
			for (unsigned n = (add_lo ? 0 : 1); n <= (num_brackets + (add_hi ? 1 : 0)); ++n) {
	...
	float tot_light_amt, vect_room_object_t &objs, colorRGBA const &pcolor, colorRGBA const &ccolor, unsigned max_shorten_steps, int add_sprinklers)
{
	...
	for (unsigned n = 0; n < max_n_iter; ++n, pipe_end += step_val) {
		...
		for (cube_t const &wall : walls) { // pipes can pass through walls, but not run inside them
			if (h_pipe.intersects(wall) && dim == (wall.dx() < wall.dy())) {inside_wall = 1; break;}
		}
		if (inside_wall) continue;
...
void building_t::get_pipe_basement_water_connections(vect_riser_pos_t &sewer, vect_riser_pos_t &cold_water, vect_riser_pos_t &hot_water, rand_gen_t &rgen) const {
	...
	if (!water_heaters.empty() && !hot_water.empty()) { // add connections for water heaters if there are hot water pipes
		float const radius_hot(get_merged_risers_radius(hot_water, PIPE_TYPE_HW)); // this is the radius of the main hot water supply; not limited to max
		// distribute evenly among the water heaters using the same merge exponent for office; use water heater pipe radius for house
		float const per_wh_radius(is_house ? WHEATER_PIPE_RADIUS*water_heaters.front().get_radius() : radius_hot*pow(1.0/water_heaters.size(), 1/4.0));
		...
		for (room_object_t const &wh : water_heaters) {
			...
			// shift to actual pipe offset from center
			wh_shift_dir[ wh.dim] = 0.0;
			wh_shift_dir[!wh.dim] = ((shift_dir[!wh.dim] < 0.0) ? -1.0 : 1.0);
			cold_water.emplace_back((center + shift_val*wh_shift_dir), per_wh_radius, 0, 1, 0, 0, 1); // has_hot=0, flow_dir=1/in , upper_floor=0, in_extb=0, is_wh=1
			hot_water .emplace_back((center - shift_val*wh_shift_dir), per_wh_radius, 1, 0, 0, 0, 1); // has_hot=1, flow_dir=0/out, upper_floor=0, in_extb=0, is_wh=1
		} // for wh
	}
...
void building_t::add_house_basement_pipes(rand_gen_t &rgen) {
	...
	for (room_object_t const &i : interior->room_geom->objs) {
		...
		if (i.type == TYPE_VENT) {
			...
			cube_t cubes[2];
			get_water_heater_cubes(i, cubes, 1); // vent_only=1
			UNROLL_2X(obstacles.push_back(cubes[i_]);)
			// add water heater pipe obstacles, only for sewage and gas pipes
