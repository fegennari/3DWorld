	...
	float radius, bool xy_only, vector<point> &points, vector3d *cnorm_ptr, bool check_interior) const
{
	...
	if (is_interior) {
		...
		for (auto i = parts.begin(); i != parts.end(); ++i) {
			...
			else if (!xy_only && i->contains_pt_xy_exp(pos2, radius) && p_last2.z > (i->z2() + xlate.z)) { // on top of building
...
bool building_t::add_door(cube_t const &c, unsigned part_ix, bool dim, bool dir, bool for_building, bool roof_access) { // exterior doors
	...
	float const pos_adj(0.025*sz[!dim]);
...
bool building_interior_t::is_cube_close_to_doorway(cube_t const &c, float dmin, bool inc_open) const { // ignores zvals
	for (auto i = doors.begin(); i != doors.end(); ++i) { // interior doors
		if (is_cube_close_to_door(c, dmin, inc_open, *i)) return 1;
	}
	return 0;
}
