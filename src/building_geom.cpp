...
float get_scaled_player_radius() {return CAMERA_RADIUS*global_building_params.player_coll_radius_scale;}
...
void building_t::clip_door_to_interior(tquad_with_ix_t &door) const {
	...
	else {xy_border = 0.06; z_border = 0.03;} // house or office door
	...
	if (num_levels == 1) { // single level
		...
		if (do_split) { // generate L, T, or U shape
				...
				min(bcube.dx(), bcube.dy()) > (is_in_city ? 11.0 : 12.0)*floor_spacing)
			{
				...
				if (num_floors >= 4 && rand_gen_t(rgen).rand_probability(global_building_params.two_floor_retail_prob)) {retail_floor_levels = 2;} // use new rgen
				parts.push_back(base);
...
void building_t::finish_gen_geometry(rand_gen_t &rgen, bool has_overlapping_cubes) { // for office buildings
	...
	if (is_datacenter()) {add_datacenter_outdoor_objs(rgen, 0);} // is_room_geom_pass=0
}
	...
	float door_center_shift, float width_scale, bool can_fail, bool opens_up, rand_gen_t &rgen, unsigned floor_ix, bool is_courtyard) const
{
	...
	for (unsigned n = 0; n < num_tries; ++n) { // make up to 10 tries to place a valid door
		...
		if (interior && (!has_pri_hall() || is_basement) && !opens_up) { // not on a hallway - check distance to interior walls to make sure the door has space to open
			...
			float const door_lo(door_center - min_wall_spacing), door_hi(door_center + min_wall_spacing); // pos along wall with a small expand
			float const dpos_lo(door_pos    -  door_half_width), dpos_hi(door_pos    +  door_half_width); // expand width of the door

			for (cube_t const &w : walls) {
...
void building_t::add_rooftop_sat_dish(cube_t const &top, float radius, rand_gen_t &rgen) {
	...
	cube_t const sd_bc(get_cube_height_radius(dish_center, radius, 3.2*radius));
		...
		if (opens_up) { // rotates inward and upward 90 degrees
			...
			if (!exterior) { // interior
				...
				for (; max_angle > 0; max_angle -= 15) { // try to open door as much as 75 degrees in steps of 15 degrees
					...
					if (!in_ext_basement && !bcube.contains_cube_xy(test_bcube)) continue; // extends outside bcube; fast/simple test; duplicate of below
					...
					cube_t tc_exp(test_bcube);
					tc_exp.expand_in_dim(!dim, 0.5*wall_thickness); // expand width for part test to avoid door edge clipping through exterior wall
					if (!in_ext_basement && !check_cube_contained_in_part(tc_exp)) continue; // extends outside part
...
float building_t::gen_peaked_roof(cube_t const &top_, float peak_height, bool dim, float extend_to, float max_dz, unsigned skip_side_tri) {
	...
	if (!is_house) {add_roof_to_bcube();} // house has already been added
	...
	if (is_restroom()) { // two doors on the ends or front side
		...
		for (unsigned d = 0; d < 2; ++d) {
			if (street_side) { // doors at front
				...
				cube_t const door(place_door(part, !dim, d, door_height, 0.0, 0.0, 0.0, DOOR_WIDTH_SCALE, 0, 0, rgen));
				if (add_door(door, 0, !dim, d, 0)) {floor_ext_door_mask |= 1;} // for_office_building=0
			}
		} // for d
		return;
	}
