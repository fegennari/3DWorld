...
void divider_t::draw(draw_state_t &dstate, city_draw_qbds_t &qbds, float dist_scale, bool shadow_only) const {
	...
	if (type == DIV_HEDGE && bcube.closest_dist_less_than(dstate.camera_bs, 0.5f*(X_SCENE_SIZE + Y_SCENE_SIZE))) {
		...
		bool const is_house(type == DIV_HOUSE_WALL);
		unsigned const skip_dirs(is_house ? (1<<unsigned(!dir)) : 0); // house walls skip the interior dir
		float const leaf_sz(0.0075*city_params.road_width);
		dstate.ivy_manager.add_wall(wall_clipped, ivy_avoid, dim, is_house, skip_dirs, divider_ix, plot_ix, city_ix, leaf_sz, dstate.camera_bs);
	}
}
	...
	: oriented_city_obj_t(pos_, max(1.25f*cradius_, 0.5f*height), dim_, dir_), color(color_), cradius(cradius_), variant(var), add_base(add_base_)
{
	...
	bcube.d[dim][ dir] += (dir ? 1.0 : -1.0)*1.25*cradius; // slightly off-center
	bcube.d[dim][!dir] -= (dir ? 1.0 : -1.0)*(add_dog_water() ? 1.10 : 0.35)*cradius;
	pos.z += 0.5*height;
}
...
/*static*/ void pond_t::pre_draw(draw_state_t &dstate, bool shadow_only) {
	assert(!shadow_only);
	select_texture(get_texture_by_name("lilypad.png")); // lily pads
	enable_blend();
}
/*static*/ void pond_t::post_draw(draw_state_t &dstate, bool shadow_only) {
	disable_blend(); // lily pads
}
	...
	cube_t const *lights, bool *cached_smaps, cube_t const *clip_cubes, unsigned num_lights, bool car_is_using) const
{
	...
	for (unsigned n = 0; n < num_lights; ++n) {
		...
		//if (clip_cubes) {dl_sources.back().set_custom_bcube(clip_cubes[n]);} // no, don't block lights spreading from the exit; shadow map should work here
		...
		dl_sources.emplace_back(0.6*ldist, lpos2, lpos2, color, 0);
		dl_sources.back().disable_shadows();
		
		if (clip_cubes) { // clip needed since there's no shadow map
			dl_sources.back().set_custom_bcube(clip_cubes[n]);
...
bool park_path_t::check_cube_coll_xy(cube_t const &c) const {
	...
	for (unsigned i = 0; i+1 < pts.size(); ++i) { // do line segment tests second
		...
		if (check_line_clip_xy(p1, p2, c.d)) return 1;

		for (unsigned n = 0; n < 4; ++n) {
			if (point_line_seg_dist_2d(point(c.d[0][n&1], c.d[1][n>>1], c.z1()), p1, p2) < hwidth) return 1;
		}
	}
	return 0;
}
