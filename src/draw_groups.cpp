	...
	const colorRGBA &color, int ndiv, int j, bool in_ammo, shader_t &shader, lt_atten_manager_t &lt_atten_manager)
{
	...
	if (is_rocket_type(type) && distance_to_camera(pos) < (CAMERA_RADIUS + 2.0*radius)) return; // too close to the camera (player just fired?)
	...
	case SFPART:
		...
		break;
	case CHUNK:
		draw_chunk(pos, radius, obj.init_dir, obj.vdeform, (obj.flags & TYPE_FLAG), (obj.flags & FROZEN_FLAG), ndiv, obj.time, shader);
		break;
	case SKULL:
...
void draw_group(obj_group &objg, shader_t &s, lt_atten_manager_t &lt_atten_manager) {
	...
	if (type == TELEPORTER) return; // drawn as a teleporter
				 ...
				 float health, int id, mesh2d const *const mesh, shader_t &shader)
{
	...
	switch (powerup) {
		case PU_DAMAGE: // devil horns
			shader.set_cur_color(RED);
			for (unsigned d = 0; d < 2; ++d) {draw_cylinder_at(point((d ? -0.35 : 0.35)*radius, 0.7*radius, 0.6*radius), 0.6*radius, 0.1*radius, 0.0, ndiv2, 0);}
			break;
		...
		case PU_FLIGHT: // propeller or wings?
			...
			for (unsigned d = 0; d < 2; ++d) {draw_sphere_vbo(point((d ? -0.5 : 0.5)*radius, 0.0, 0.0), 0.5*radius, ndiv, 0);} // propeller
			fgPopMatrix();
			break;
	...
	else if (sstates[id].freeze_time > 0 || temperature < 0.75*object_types[SMILEY].min_t) { // frozen
		shader.set_cur_color(colorRGBA(FREEZE_COLOR, alpha)); // light blue
	}
...
void draw_rapt_proj(point const &pos, vector3d const &orient, float radius, int type, int ndiv, int time, shader_t &shader) {
	...
	draw_cylinder(-1.1, 1.0, 1.0, ndiv);
	fgPopMatrix();
	if ((type == RAPT_PROJ || type == FREEZE_BOMB) && time > 0.1*TICKS_PER_SECOND) {gen_rocket_smoke(pos, orient, 0.8*radius, (type == FREEZE_BOMB));}
}
...
void draw_chunk(point const &pos, float radius, vector3d const &v, vector3d const &vdeform, int charred, int frozen, int ndiv, int time, shader_t &shader) {
	...
	else if (frozen) { // transition from frozen to normal
		float const val((float)time/(float)object_types[CHUNK].lifetime);
		c1 = blend_color(YELLOW,  colorRGBA(0.5, 0.7, 1.0, 1.0), val, 0);
		c2 = blend_color(BLOOD_C, colorRGBA(0.2, 0.4, 1.0, 1.0), val, 0);
	}
