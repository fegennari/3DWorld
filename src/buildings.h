...
struct building_params_t {
	...
	float wall_split_thresh=4.0, max_fp_wind_xscale=0.0, max_fp_wind_yscale=0.0, basement_water_level=0.0; // interiors
...
enum {
	...
	TYPE_HOOD, TYPE_RCHAIR, TYPE_SILVER, TYPE_TOY_MODEL, TYPE_CEIL_FAN, TYPE_FIRE_EXT, TYPE_FOLD_SHIRT, TYPE_PLANT_MODEL, TYPE_POOL_TABLE,
	/* animals */
...
enum {RTYPE_NOTSET=0, RTYPE_HALL, RTYPE_STAIRS, RTYPE_OFFICE, RTYPE_BATH, RTYPE_BED, RTYPE_KITCHEN, RTYPE_LIVING, RTYPE_DINING, RTYPE_STUDY,
	  ...
	  RTYPE_UTILITY, RTYPE_PARKING, RTYPE_RAMP_EXIT, RTYPE_ATTIC, RTYPE_MASTER_BED, RTYPE_UNFINISHED, RTYPE_SERVER, RTYPE_POOL, RTYPE_BACKROOMS, NUM_RTYPES};
...
std::string const room_names[NUM_RTYPES] =
	{"Unset", "Hallway", "Stairs", "Office", "Bathroom", "Bedroom", "Kitchen", "Living Room", "Dining Room", "Study",
	 ...
	 "Utility Room", "Parking Garage", "Ramp Exit", "Attic", "Master Bedroom", "Unfinished Room", "Server Room", "Pool Room", "Backrooms"};
...
enum {/*building models*/ OBJ_MODEL_TOILET=0, OBJ_MODEL_SINK, OBJ_MODEL_TUB, OBJ_MODEL_FRIDGE, OBJ_MODEL_STOVE, OBJ_MODEL_TV, OBJ_MODEL_MONITOR, OBJ_MODEL_COUCH,
	...
	OBJ_MODEL_FOLD_SHIRT, OBJ_MODEL_PLANT, OBJ_MODEL_POOL_TABLE, OBJ_MODEL_RAT, OBJ_MODEL_ROACH,
...
struct building_room_geom_t {
	...
	void add_pool_table(room_object_t const &c);
...
struct door_base_t : public cube_t {
	bool dim=0, open_dir=0, hinge_side=0, on_stairs=0, mult_floor_room=0;
...
struct ext_step_t : public cube_t {
	bool dim, step_dir, wall_dir, at_door, is_base;
	ext_step_t(cube_t const &c, bool dim_, bool sdir, bool wdir, bool door=0, bool base=0) :
		cube_t(c), dim(dim_), step_dir(sdir), wall_dir(wdir), at_door(door), is_base(base) {}
};
...
struct building_t : public building_geom_t {
	...
	void get_basment_ext_wall_verts  (building_draw_t &bdraw) const;
	...
	bool get_nearby_ext_door_verts(building_draw_t &bdraw, shader_t &s, point const &pos, float dist, bool reflection_pass=0);
...
public:
	...
	bool place_obj_along_wall(room_object type, room_t const &room, float height, vector3d const &sz_scale, rand_gen_t &rgen, float zval, unsigned room_id,
		float tot_light_amt, cube_t const &place_area, unsigned objs_start, float front_clearance=0.0, bool add_door_clearance=0,
...
private:
	...
	bool is_valid_stairs_elevator_placement(cube_t const &c, float pad, int dim=2, bool check_walls=1) const;
	...
	void add_breaker_panel   (rand_gen_t &rgen, cube_t const &c, float bp_hdepth, float ceil_zval, bool dim, bool dir, unsigned room_id, float tot_light_amt);
