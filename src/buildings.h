...
struct building_params_t {
	...
	float wall_split_thresh, max_fp_wind_xscale, max_fp_wind_yscale, open_door_prob, locked_door_prob, basement_prob_house, basement_prob_office, ball_prob; // interiors
	// building AI params
	...
	unsigned people_per_office_min, people_per_office_max, people_per_house_min, people_per_house_max;
	...
	building_params_t(unsigned num=0) : flatten_mesh(0), has_normal_map(0), tex_mirror(0), tex_inv_y(0), tt_only(0), infinite_buildings(0), dome_roof(0),
		...
		wall_split_thresh(4.0), max_fp_wind_xscale(0.0), max_fp_wind_yscale(0.0), open_door_prob(1.0), locked_door_prob(0.0), basement_prob_house(0.5),
		basement_prob_office(0.5), ball_prob(0.3), ai_target_player(1), ai_follow_player(0), ai_opens_doors(1), ai_player_vis_test(0),
		people_per_office_min(0), people_per_office_max(0), people_per_house_min(0), people_per_house_max(0), num_rats_min(0),
		num_rats_max(0), min_attack_rats(0), num_spiders_min(0), num_spiders_max(0), rat_speed(0.0), rat_size_min(0.5), rat_size_max(1.0), spider_speed(0.0),
		spider_size_min(0.5), spider_size_max(1.0), spider_drawer_prob(0.0), player_weight_limit(100.0), range_translate(zero_vector), read_error(0),
...
enum {
	...
	TYPE_CURB, TYPE_BRK_PANEL, TYPE_VENT,
...
public:
	unsigned num_verts, num_ixs, vert_vbo_sz, ixs_vbo_sz, verts_hashval; // for drawing
	uint8_t dir_mask; // {-x, +x, -y, +y, -z, +z}
	bool en_shadows;

	rgeom_mat_t(tid_nm_pair_t const &tex_=tid_nm_pair_t()) : rgeom_storage_t(tex_),
		num_verts(0), num_ixs(0), vert_vbo_sz(0), ixs_vbo_sz(0), verts_hashval(0), dir_mask(0), en_shadows(0) {}
...
struct building_t : public building_geom_t {
	...
	bool is_house, has_garage, has_shed, has_int_garage, has_courtyard, has_complex_floorplan, has_helipad, has_ac, has_int_fplace, has_parking_garage, has_small_part;
	...
	building_t(unsigned mat_ix_=0) : mat_ix(mat_ix_), hallway_dim(2), real_num_parts(0), roof_type(ROOF_TYPE_FLAT), street_dir(0), open_door_ix(-1),
		...
		has_helipad(0), has_ac(0), has_int_fplace(0), has_parking_garage(0), has_small_part(0), side_color(WHITE), roof_color(WHITE), detail_color(BLACK),
		door_color(WHITE), wall_color(WHITE), ao_bcz2(0.0), ground_floor_z1(0.0) {}
	building_t(building_geom_t const &bg) : building_geom_t(bg), mat_ix(0), hallway_dim(2), real_num_parts(0), roof_type(ROOF_TYPE_FLAT), street_dir(0),
		...
		has_complex_floorplan(0), has_helipad(0), has_ac(0), has_int_fplace(0), has_parking_garage(0), has_small_part(0), side_color(WHITE),
		roof_color(WHITE), detail_color(BLACK), door_color(WHITE), wall_color(WHITE), ao_bcz2(0.0), ground_floor_z1(0.0) {}
...
private:
	...
	void try_place_light_on_ceiling(cube_t const &light, room_t const &room, bool room_dim, float pad, bool allow_rot, bool allow_mult, vect_cube_t &out, rand_gen_t &rgen);
	...
	bool add_water_heaters   (rand_gen_t &rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start);
	...
	bool add_vent_to_room   (rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, unsigned objs_start);
