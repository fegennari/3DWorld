...
struct building_params_t {
	...
	bool gen_building_interiors, add_city_interiors, enable_rotated_room_geom, add_secondary_buildings;
	...
	building_params_t(unsigned num=0) : flatten_mesh(0), has_normal_map(0), tex_mirror(0), tex_inv_y(0), tt_only(0), infinite_buildings(0), dome_roof(0),
		...
		num_place(num), num_tries(10), cur_prob(1), max_shadow_maps(32), buildings_rand_seed(0), ao_factor(0.0), sec_extra_spacing(0.0), player_coll_radius_scale(1.0),
		interior_view_dist_scale(1.0), window_width(0.0), window_height(0.0), window_xspace(0.0), window_yspace(0.0), wall_split_thresh(4.0), max_fp_wind_xscale(0.0),
		max_fp_wind_yscale(0.0), open_door_prob(1.0), locked_door_prob(0.0), basement_prob(0.5), ball_prob(0.3),
...
enum {RTYPE_NOTSET=0, RTYPE_HALL, RTYPE_STAIRS, RTYPE_OFFICE, RTYPE_BATH, RTYPE_BED, RTYPE_KITCHEN, RTYPE_LIVING, RTYPE_DINING, RTYPE_STUDY, RTYPE_ENTRY,
	  RTYPE_LIBRARY, RTYPE_STORAGE, RTYPE_GARAGE, RTYPE_SHED, RTYPE_LOBBY, RTYPE_LAUNDRY, RTYPE_CARD, RTYPE_PLAY, RTYPE_ART, NUM_RTYPES};
...
struct building_interior_t {
	...
	bool is_cube_close_to_doorway(cube_t const &c, cube_t const &room, float dmin=0.0f, bool inc_open=0) const;
...
struct building_t : public building_geom_t {
	...
	building_t(unsigned mat_ix_=0) : mat_ix(mat_ix_), hallway_dim(2), real_num_parts(0), roof_type(ROOF_TYPE_FLAT), open_door_ix(-1), basement_part_ix(-1),
		is_house(0), has_chimney(0), has_garage(0), has_shed(0), has_int_garage(0), has_courtyard(0), has_complex_floorplan(0), has_helipad(0), has_ac(0),
	...
	bool has_dw_porch () const {return !driveway.is_all_zeros();}
	bool has_driveway () const {return (has_garage && has_dw_porch());}
	bool enable_driveway_coll() const {return (has_dw_porch() && !is_rotated());} // no collision with rotated driveways/porches for now
	...
	bool maybe_add_house_driveway(cube_t const &plot, vect_cube_t &driveways) const;
