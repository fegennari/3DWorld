...
struct riser_pos_t : public sphere_t {
	...
	riser_pos_t() {}
	riser_pos_t(point const &pos_, float radius_, bool hh=0, bool fd=0, bool uf=0, bool eb=0, bool wh=0) :
		sphere_t(pos_, radius_), has_hot(hh), flow_dir(fd), upper_floor(uf), in_extb(eb), is_wh(wh) {}
};
...
struct room_object_t : public oriented_cube_t { // size=68
	...
	bool is_horizontal_cylin   () const {return (shape == SHAPE_CYLIN && !is_vert_cylinder());}
...
public:
	...
	void add_round_rect_to_verts(cube_t const &c, float corner_radius, colorRGBA const &color, bool draw_top=1, bool draw_bot=1,
		bool skip_sides=0, bool two_sided=0, float rs_bot=1.0, float rs_top=1.0, unsigned ndiv=N_CYL_SIDES);
...
struct building_room_geom_t {
	...
	rgeom_mat_t &get_rafters_material(float tscale, bool inc_shadows, unsigned small, bool is_painted);
	...
	void add_flat_textured_detail_wall_object(room_object_t const &c, colorRGBA const &side_color, int tid, bool skip_z1_face,
		bool draw_all_faces=0, bool detail=1, bool mirror_y=0, float spec=0.0, float shine=0.0, float metalness=0.0, bool exterior=0);
	void add_outlet(room_object_t const &c);
	void add_vent(room_object_t const &c, bool add_exterior=0);
	...
	void add_boiler(room_object_t const &c);
	...
	void expand_comm_fridge(room_object_t const &c);
	...
	room_object_t get_item_in_drawer(room_object_t const &c, cube_t const &drawer, unsigned drawer_ix, unsigned item_ix, float &stack_z1) const;
	void add_draw_items(room_object_t const &c, cube_t const &drawer, unsigned drawer_ix, vect_room_object_t &objects) const;
	...
	void replace_with_hanging_wires(room_object_t &obj, room_object_t const &old_obj, float wire_radius, bool vertical, float shorten_amt=0.1);
...
enum {ROOF_OBJ_BLOCK=0, ROOF_OBJ_ANT, ROOF_OBJ_WALL, ROOF_OBJ_ECAP, ROOF_OBJ_AC, ROOF_OBJ_SCAP, ROOF_OBJ_SIGN, ROOF_OBJ_SIGN_CONN, ROOF_OBJ_WTOWER, ROOF_OBJ_DUCT,
	ROOF_OBJ_SMOKESTACK, ROOF_OBJ_SAT_DISH, ROOF_OBJ_TV_ANT, ROOF_OBJ_COOLING, DETAIL_OBJ_COLLIDER, DETAIL_OBJ_COLL_SHAD, DETAIL_OBJ_SHAD_ONLY, DETAIL_OBJ_KEEPOUT};
...
struct building_interior_t {
	...
	void remove_objects_in_room(unsigned room_id, unsigned objs_end_ix, float z1, float z2, vect_cube_t &lights_bcubes);
...
struct building_t : public building_geom_t {
	...
	bool has_tline_conn=0, has_smokestack=0, has_antenna=0, has_radiators=0, was_custom_placed=0, mw_restroom_side=0;
	...
	bool pri_hall_stairs_to_pg=0, have_walkway_ext_door=0, have_hall_side_stairs=0;
	...
	mutable cube_t city_driveway, city_walkway; // set by city gen, which only has a const ref to the building; cached city state, and not directly used by the building
	...
	bool room_inc_half_walls(room_t const &room) const;
	...
	unsigned get_num_main_parts() const {return (real_num_parts - has_basement());} // above ground, excludes basement, garage, and shed
	...
	void add_smokestacks(rand_gen_t &rgen);
	...
	int calc_floor_offset(float zval, float floor_spacing) const;
...
public:
	...
	bool place_obj_along_wall(room_object type, room_t const &room, float height, vector3d const &sz_scale, rand_gen_t &rgen, float zval, unsigned room_id,
		...
		unsigned extra_flags=0, bool not_ext_wall=0, bool force_pref=0, unsigned skip_walls_mask=0);
	...
	bool is_cube_visible(cube_t const &c, vector3d const &xlate, bool inc_mirror_reflections=0) const;
	...
	float get_room_light_amt(room_t const &room) const;
...
private:
	...
	void add_desk_objects(rand_gen_t &rgen, unsigned desk_obj_ix, colorRGBA const &chair_color, room_t const &room,
		vect_cube_t const &blockers, bool add_computer=0, bool add_phone=0, float comp_sz_scale=1.0);
	...
	void add_op_center_objs(rand_gen_t rgen, room_t const &room, colorRGBA const &chair_color, float zval, unsigned room_id, unsigned floor_ix, float tot_light_amt, unsigned objs_start);
	...
	bool add_bed_to_room     (rand_gen_t &rgen, room_t const &room, vect_cube_t const &blockers, float zval, unsigned room_id,
		float tot_light_amt, unsigned floor, bool force, int &bed_size_ix, room_object_t const &other_bed, int cand_bath=-1);
	...
	void add_valve_and_gauge_panel(rand_gen_t &rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start);
	...
	void add_play_room_objs  (rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start);
	void add_art_room_objs   (rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start);
	...
	void add_dc_utility_objs (rand_gen_t rgen, room_t const &room, float &zval, unsigned room_id, unsigned floor_ix,
		unsigned num_floors, float tot_light_amt, unsigned objs_start, unsigned lights_start);
	bool add_server_room_objs(rand_gen_t rgen, room_t const &room, float &zval, unsigned room_id, float tot_light_amt, unsigned objs_start, unsigned lights_start);
	...
	bool add_breaker_panel_by_door(rand_gen_t &rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt);
	...
	bool add_row_of_models(cube_t const &place_area, float zval, unsigned room_id, float tot_light_amt, float height, float gap_mult,
	...
	bool add_corner_trashcans(rand_gen_t &rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start, bool dim, bool both_ends);
	...
	bool connect_steam_pipe_to_machine(rand_gen_t &rgen, cube_t &pipe, colorRGBA const &color, float light_amt,
	...
	cube_t get_water_users_bcube() const;
	...
	bool place_stapler_on_obj(rand_gen_t &rgen, cube_t const &place_on, unsigned room_id, float tot_light_amt, vect_cube_t const &avoid=vect_cube_t());
	...
	void add_outlets_to_room  (rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, unsigned objs_start,
		bool is_ground_floor, bool is_basement, bool is_kitchen=0, bool is_bathroom=0);
	...
	bool place_tv_static_on_surface(rand_gen_t &rgen, cube_t const &surface, unsigned room_id, float tot_light_amt, unsigned objs_start);
	...
	void refine_light_bcube_pg_walls(point const &lpos, cube_t &light_bcube) const;
...
void get_city_road_segs_in_region(cube_t const &region, vect_cube_t &out);
