...
struct building_params_t {
	...
	building_params_t(unsigned num=0) : flatten_mesh(0), has_normal_map(0), tex_mirror(0), tex_inv_y(0), tt_only(0), infinite_buildings(0), dome_roof(0),
		...
		num_place(num), num_tries(10), cur_prob(1), max_shadow_maps(32), ao_factor(0.0), sec_extra_spacing(0.0), player_coll_radius_scale(1.0), interior_view_dist_scale(1.0),
		...
		open_door_prob(1.0), locked_door_prob(0.0), basement_prob(0.5), ball_prob(0.3),
...
enum {
	...
	TYPE_DRAIN, TYPE_MONEY, TYPE_PHONE, TYPE_TPROLL, TYPE_SPRAYCAN,
...
private:
	...
	void add_tp_roll         (cube_t const &room, unsigned room_id, float tot_light_amt, bool dim, bool dir, float length, float zval, float wall_pos);
...
void draw_building_interior_spraypaint(building_t const *const building=nullptr);
