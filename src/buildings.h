...
enum room_object    {
	...
	TYPE_RAILING, TYPE_CRATE, TYPE_MIRROR, TYPE_SHELVES, TYPE_KEYBOARD, TYPE_DRIVEWAY,
...
struct obj_model_inst_t {
	unsigned obj_id, model_id, flags;
	colorRGBA color;
	vector3d dir;
	obj_model_inst_t(unsigned oid, unsigned mid, unsigned f, colorRGBA const &c, vector3d const &d) : obj_id(oid), model_id(mid), flags(f), color(c), dir(d) {}
};
...
struct building_room_geom_t {
	...
	void add_driveway(room_object_t const &c, float tscale);
...
struct building_t : public building_geom_t {
	...
	building_t(unsigned mat_ix_=0) : mat_ix(mat_ix_), hallway_dim(2), real_num_parts(0), roof_type(ROOF_TYPE_FLAT), is_house(0), has_chimney(0), has_garage(0),
		has_shed(0), has_courtyard(0), has_complex_floorplan(0), has_helipad(0), side_color(WHITE), roof_color(WHITE), detail_color(BLACK), door_color(WHITE), ao_bcz2(0.0) {}
	building_t(building_geom_t const &bg) : building_geom_t(bg), mat_ix(0), hallway_dim(2), real_num_parts(0), roof_type(ROOF_TYPE_FLAT),
		is_house(0), has_chimney(0), has_garage(0), has_shed(0), has_courtyard(0), has_complex_floorplan(0), has_helipad(0), door_color(WHITE), ao_bcz2(0.0) {}
