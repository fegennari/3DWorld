...
float triangle_area(point const &p1, point const &p2, point const &p3) {
	...
	float const val((a+b+c)*(b+c-a)*(c+a-b)*(a+b-c));
	return 0.25*sqrt(max(val, 0.0f)); // handle negative area from degenerate triangles
}
...
bool line_intersect_sphere(point const &p1, vector3d const &v12, point const &sc, float radius, float &rad, float &dist, float &t) {
	...
	if (dotp < 0.0f && dist > radius) return 0; // pointing away from sphere and not inside it
	t   = dotp/dist; // t must be <= 1.0, assuming v12 is normalized
	rad = sqrt(1.0f - min(t*t, 1.0f))*dist; // rad in (-dist, dist): sin(acos(t))*dist
	return (radius > rad);
}


// pl = starting point of the line, sc = sphere center, v1 = line direction (inverted?), r2sq = square of the sphere radius
template<typename T> bool sphere_test_comp(pointT<T> const &pl, pointT<T> const &sc, pointT<T> const &v1, T r2sq, T &t) { // line segment/sphere intersection

	pointT<T> const v2(pl, sc); // sphere center to line start
