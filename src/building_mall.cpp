...
void building_t::setup_mall_concourse(cube_t const &room, bool dim, bool dir, rand_gen_t &rgen) {
	...
	for (unsigned f = 1; f < num_floors; ++f) { // skip first floor
		...
		for (unsigned n = 0; n <= num_openings; ++n) { // ends and gaps between openings
			...
			float const delta_z(floor_spacing + 2.0*trim_thickness), e_width(0.9*door_width); // make slightly taller so that top surface is above the railing bottom trim
			cube_t epair;
...
void building_t::add_mall_store(cube_t const &store, cube_t const &window_area, bool dim, bool dir, bool &has_adj_store, vect_cube_t &wall_cuts, rand_gen_t &rgen) {
	...
	wall_cut.expand_in_dim(dim, -(0.25*window_vspace + 0.15*window_area.get_sz_dim(dim))*(at_mall_end ? 0.25 : 1.0)); // shrink, less at mall end stores
	set_wall_width(wall_cut, wall_pos, 2.0*wall_thickness, !dim); // add extra thickness
	wall_cuts.push_back(wall_cut);
...
void building_t::add_mall_stores(cube_t const &room, bool dim, bool entrance_dir, rand_gen_t &rgen) { // and bathrooms
	...
	for (unsigned f = 0; f < num_floors; ++f) {
			...
			while (pos + min_width < pos_end) { // continue until we can't fit a min width room
				...
				if (is_bathroom) {
					...
					for (unsigned e = 0; e < 2; ++e) {
						...
						wall_cuts.push_back(door);
						wall_cuts.back().expand_in_dim(!dim, wall_thickness);
					} // for e
					...
					add_mall_store(store, store, dim, d, has_adj_store, wall_cuts, rgen); // window_area is full storefront
				}
				floor_bcube.union_with_cube(store);
				pos = next_pos;
			} // while
			subtract_cubes_from_cubes(wall_cuts, side_walls, 1); // clip_in_z=1
		} // for d
		...
		for (unsigned d = 0; d < 2; ++d) { // ends of mall
			...
			wall_cuts.clear();
			add_mall_store(store, window_area, !dim, d, has_adj_store, wall_cuts, rgen);
			subtract_cubes_from_cubes(wall_cuts, end_walls, 1); // clip_in_z=1
			floor_bcube.union_with_cube(store);
			added_end_store[d] = 1;
		} // for d
...
// Note: also used with school and prison cafeterias
...
unsigned building_t::add_mall_store_objs(rand_gen_t rgen, room_t &room, float zval, unsigned room_id, unsigned &type_mask, cube_t &div_wall, light_ix_assign_t &light_ix_assign) {
	...
	if (!is_end_store && !fc_area.is_zero_area() && room.z1() == fc_area.z1()) { // food stores should be placed on ground floors near the food court
		// always place if center is contained in food court
		float const center_mdim(room.get_center_dim(mall_dim));
		if (center_mdim > fc_area.d[mall_dim][0] && center_mdim < fc_area.d[mall_dim][1]) {store_type = STORE_FOOD;}
	...
	bool no_doorway, float light_amt, cube_t &div_wall, light_ix_assign_t &light_ix_assign)
{
	...
	float const fb_split(front_wall + (dir ? -1.0 : 1.0)*((has_dining ? 0.5 : 0.25) + rgen.rand_uniform(0.0, 0.1))*room.get_sz_dim(dim)); // front vs. back split pos
