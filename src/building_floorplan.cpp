...
void building_t::gen_interior_int(rand_gen_t &rgen, unsigned gen_index, bool has_overlapping_cubes) { // Note: contained in building bcube, so no bcube update is needed
	...
	for (auto p = parts.begin(); p != parts_end; ++p) {
		...
		bool const is_restroom_part  (is_restroom  () && first_part);
		...
		if (!is_cube()) { // cylinder, etc.
			...
			while (!to_split.empty()) { // recursively split rooms until all rooms are too small to split or there are no more valid splits
				...
				for (unsigned dim_pass = 0; dim_pass < num_dim_passes; ++dim_pass) { // if room is near square and can't split in one dim, can try the other dim
					...
					for (unsigned num = 0; num < 100; ++num) { // try to choose a wall pos that's not inside a window or skylight or intersecting a wall
	...
	for (unsigned d = 0; d < 2; ++d) { // x,y: dim in which the wall partitions the room (wall runs in dim !d)
		...
		for (unsigned w = first_wall_to_split[d]; w < walls.size(); ++w) { // skip hallway walls
			...
			for (unsigned nsplits = 0; nsplits < 4; ++nsplits) { // at most 4 splits
				...
				// don't split warehouse/factory/restaurant walls as they already have doors
				if ((is_industrial() || is_restaurant() || is_conv_store()) && wall.z1() >= ground_floor_z1) break;
	...
	unsigned rooms_start, bool use_hallway, bool first_part_this_stack, float window_hspacing[2], float window_border, bool is_single_floor)
{
	...
	if (!has_roof_access && !is_restroom_with_high_ceil()) { // roof ceiling, full area
		...
		if ((is_house || is_restaurant()) && part_ix == get_attic_part_ix() && add_attic_access_door(C, part_ix, num_floors, rooms_start, rgen)) {
...
void building_t::connect_stacked_parts_with_stairs(rand_gen_t &rgen, cube_t const &part, unsigned lower_part_ix) { // and extend elevators vertically; part is lower
	...
	if (part.z2() < bcube.z2()) { // if this is the top floor, there is nothing above it (but roof geom may get us into this case anyway)
		...
		for (unsigned upper_part_ix = 0; upper_part_ix < real_num_parts; ++upper_part_ix) { // find the part on the top
			...
			else if (!(is_basement && (house_fplan || is_parking())) && is_cube()) { // not for house/restaurant basements, parking garages, or non-cube buildings
			...
			for (unsigned n = 0; n < cur_num_iters && !cand_is_valid; ++n) { // make up to 200 tries to add stairs
				...
				bool const allow_clip_walls(n >= 180 && !house_fplan && !is_prison()), use_pref_shared(n <= 2*iter_mult_factor);
				...
				bool const pri_hall_stairs(is_basement && !house_fplan && has_pri_hall() && pri_hall.z1() == ground_floor_z1 && dim == (hallway_dim == 1));
		...
		for (unsigned s = 0; s < 2; ++s) { // for each side of the door
			...
			if (ds_room_ix == -1) { // adj room not found
				if (d->get_bldg_conn()) { // door connecting adjacent building with no room for this building on the other side
					...
					if (ds_room_ix < 0) {cout << "bad door " << door_center.str() << endl;}
					assert(ds_room_ix >= 0);
