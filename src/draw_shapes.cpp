...
void cobj_draw_buffer::draw_cylin_cdb(point const &p1, point const &p2, texgen_params_t const &tp,
	float radius1, float radius2, int ndiv, bool texture, bool draw_sides_ends, bool two_sided_lighting)
{
	...
	if (radius2 == 0.0) {gen_cone_triangles(tc_tri_verts, vpn, two_sided_lighting);} // cone FIXME: tp
...
void coll_obj::draw_extruded_polygon(int tid, cobj_draw_buffer &cdb) const {
	...
	bool const treat_as_cube(was_a_cube());
	
	if (!treat_as_cube && thick <= MIN_POLY_THICK) { // double_sided = 0, relies on points being specified in the correct CW/CCW order
		...
		for (unsigned fi = 0; fi < nsides; ++fi) { // draw back to front
			...
			if (s < 2) { // draw back and front
				...
				if (treat_as_cube) { // the primary polygon faces were the original cube top and bottom faces
				...
				if (!bfc || !camera_behind_polygon(side_pts, 4)) {
					...
					if (treat_as_cube) { // {x1,y1 x2,y1 x2,y2 x1,y2} => {-y +x +y -x}
