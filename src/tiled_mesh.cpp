...
size_t tile_t::get_gpu_mem() const {
	...
	size_t mem(pine_trees.get_gpu_mem() + decid_trees.get_gpu_mem() + scenery.get_gpu_mem() + flowers.get_gpu_mem());
...
size_t tile_t::get_smap_mem() const {
	size_t mem(0);
...
size_t tile_shadow_map_manager::get_free_list_mem_usage() const {
	size_t mem(0);
...
void tile_t::setup_shadow_maps(tile_shadow_map_manager &smap_manager, bool cleanup_only) {
	...
	if (!models_bcube.is_all_zeros()) {max_eq(bcube.z2(), models_bcube.z2());}
...
void tile_t::create_texture(mesh_xy_grid_cache_t &height_gen) {
	...
	//highres_timer_t timer("Create Tile Weights Texture"); // 509 288.738 4.367 0.567265 | 521 447.42 12.5631 0.858773
	assert(zvals.size() == zvsize*zvsize);
...
#pragma omp parallel for schedule(static,1) num_threads(2)
		...
		if (has_city_grass && sz_factor > 1) { // increase weights texture resolution to more accurately control grass placement within cities
			//highres_timer_t timer("Create Tile Weights Grass"); // 26 35.309 2.5545 1.35804 | 27 114.814 8.9916 4.25236
...
void tile_t::draw_mesh_vbo(indexed_vbo_manager_t const &vbo_mgr, unsigned const ivbo_ixs[NUM_LODS+1], unsigned lod_level) const {
	...
	draw_indexed_tri_verts(stride*stride, num_ixs, GL_TRIANGLE_STRIP, (void *)(start_ix*sizeof(unsigned)));
}
...
void tile_t::draw(shader_t &s, indexed_vbo_manager_t const &vbo_mgr, unsigned const ivbo_ixs[NUM_LODS+1], crack_ibuf_t const &crack_ibuf, int reflection_pass, int shader_locs[2]) const {
	...
	for (unsigned dim = 0; dim < 2; ++dim) { // x,y
		for (unsigned dir = 0; dir < 2; ++dir) { // lo, hi
			...
			draw_indexed_tri_verts(stride*stride, ixsz.sz, GL_TRIANGLES, (void *)(ixsz.ix*sizeof(unsigned)));
		} // for dir
	} // for dim
