...
struct car_t : public car_base_t, public waiting_obj_t { // size = 136
	...
	uint8_t color_id=0, front_car_turn_dir=TURN_UNSPEC, model_id=0, headlight_color=0, dest_gs_lane=0, dest_cw_lane=0, park_btype=BTYPE_UNSET;
...
struct road_plot_t : public cube_t {
	...
	bool is_residential=0, has_parking=0, is_park=0, has_pond=0, no_draw=0;
...
struct ivy_wall_t {
	...
	void gen(cube_t const &wall, vect_cube_t const &avoid, float leaf_sz, unsigned face_mask, bool rand_select, rand_gen_t &rgen);
	void place_on_wall_face(cube_t const &wall, vect_cube_t const &avoid, bool dim, bool dir, float leaf_sz,
...
public:
	...
	void clear();
	void add_wall(cube_t const &wall, vect_cube_t const &avoid, bool dim, unsigned skip_dirs, unsigned wall_ix, unsigned plot_ix, unsigned city_ix, float leaf_sz, point const &camera_bs);
	void draw_and_clear(shader_t &s);
};
...
struct pedestrian_t : public person_base_t { // city pedestrian
	...
	bool try_place_in_plot(road_plot_t const &cand_plot, vect_cube_t const &colliders, unsigned plot_id, rand_gen_t &rgen);
