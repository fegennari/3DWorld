...
bool building_t::player_can_see_outside() const {
	...
	for (auto const &s : interior->stairwells) { // check basement stairs
		...
		// assume exterior may be visible through normal stairs, but not parking garage stairs, unless it's industrial (maybe not needed, since no ground floor windows?)
		if (!has_parking_garage || is_industrial()) return 1;
		cube_t stairs_exp(s);
...
float room_object_t::get_radius() const {
	if (shape == SHAPE_CYLIN || shape == SHAPE_VERT_TORUS) { // vertical cylinder or torus: return average of x/y diameter
		if (type == TYPE_POOL_CUE || type == TYPE_SHELL_CASE) return 0.5*min_len();
...
unsigned check_vent_hood_collision(room_object_t const &c, point &pos, point const &p_last, float radius, vector3d *cnorm) {
...
bool building_t::check_pos_in_unlit_room_recur(point const &pos, set<unsigned> &rooms_visited, int known_room_id) const {
	...
	for (auto i = interior->room_geom->objs.begin(); i != objs_end; ++i) {
		...
		if ((i->is_light_type   () && i->is_light_on     () /*&& !i->light_is_out()*/) || // light that's turned on; include broken lights so that sparks are drawn
			(i->is_tv_or_monitor() && i->is_tv_monitor_on())) // TV or monitor emits light
		{
...
bool building_t::is_room_windowless(room_t const &room) const { // or interior; skylights don't count
...
public:
	void gen_objs(building_t const &building, float z1, float z2) {
		...
		// add elevator shaft, in case elevator isn't on this floor
		for (elevator_t  const &e : building.interior->elevators ) {add_obj(cube_with_color_t(e, GRAY_BLACK, 0));} // is_round=0
		for (stairwell_t const &s : building.interior->stairwells) {add_obj(cube_with_color_t(s, LT_GRAY,    0));} // is_round=0
		...
		for (room_object_t const &obj : building.interior->room_geom->objs) { // add room objects
			...
			if (type == TYPE_STAIR || type == TYPE_BOOK || type == TYPE_PLANT || type == TYPE_RAILING || obj.is_a_drink() || type == TYPE_PAPER || type == TYPE_PAINTCAN ||
...
bool building_t::overlaps_other_room_obj(cube_t const &c, unsigned objs_start, bool check_all, unsigned const *objs_end) const {
	...
	for (auto i = start; i != end; ++i) {
		...
		if ((check_all || !i->no_coll() || type == TYPE_PIPE || type == TYPE_FALSE_DOOR || type == TYPE_FIRE_EXT || type == TYPE_TV || type == TYPE_MONITOR ||
			type == TYPE_US_FLAG || type == TYPE_CLOCK || type == TYPE_SHOE || i->is_pet_container()) && i->intersects_no_adj(c)) return 1;
...
bool room_object_t::is_vert_cylinder() const {
	...
	if (type == TYPE_BUTTON || type == TYPE_HANGER_ROD || type == TYPE_CLOCK || type == TYPE_VALVE || type == TYPE_METAL_BAR || type == TYPE_BOILER) return 0; // horizontal cylinders
	if (type != TYPE_DUCT   && type != TYPE_PIPE)        return 1; // only ducts and pipes can be either horizontal or vertical cylinders
...
void building_t::get_room_obj_cubes(room_object_t const &c, point const &pos, vect_cube_t &lg_cubes, vect_cube_t &sm_cubes, vect_cube_t &non_cubes) const {
	...
	if (c.is_round() || c.is_floor_clutter()) {non_cubes.push_back(c);}
		...
		type == TYPE_SHOWERTUB || type == TYPE_CEIL_TILE || type == TYPE_WALL_GAP || type == TYPE_SINK || type == TYPE_EASEL || type == TYPE_WHEELCHAIR ||
		type == TYPE_GENERATOR || type == TYPE_WETF_SIGN)
	{
...
bool building_t::is_over_uncovered_floor(point const &pos_bs) const { // for floor is lava
	...
	for (cube_t const &f : interior->floors) {
		...
		for (auto const &obj : interior->room_geom->objs) {
			if (!obj.intersects(c)) continue;
			if (obj.type == TYPE_RUG || obj.type == TYPE_FLOORING || obj.type == TYPE_STAIR) {on_obj = 1; break;}
		}
		if (!on_obj) return 1;
	} // for c
	return 0;
...
bool building_t::is_cube_visible_through_extb_door(point const &viewer, cube_t const &c) const {
	...
	door_bc.expand_by_xy(2.0*get_wall_thickness()); // standing in the doorway counts; add extra padding
	...
	for (unsigned p = 0; p < min(4U, (unsigned)real_num_parts); ++p) {
		for (unsigned dim = 0; dim < 2; ++dim) {
			for (unsigned dir = 0; dir < 2; ++dir) {
				if (door_sides[p] & (1U << (2*dim + dir))) continue; // skip walls with doors
				cube_t const &part(parts[p]);
				if (part.z1() != ground_floor_z1) continue; // skip upper parts and basements
				float const wall_pos(bcube.d[dim][dir]), wall_thick(get_wall_thickness());
