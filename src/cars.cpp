...
void car_t::person_in_the_way(bool is_player, bool at_stopsign) {
	if (!in_parking_lot && cur_road_type != TYPE_DRIVEWAY) { // don't honk if in a parking lot or driveway, since people have a right to walk there
		...
		unsigned const rgen_mod_val(is_player ? 2 : (is_zombie ? 50 : (at_stopsign ? 25 : 16)));
...
bool is_active_emergency_vehicle(car_model_loader_t const &car_model_loader, car_t const &car, bool lights, bool siren) {
	...
	unsigned const active_mod(lights ? 5 : 15); // lights 20% of the time, sirens 6.7% of the time; siren mod must be a multiple of lights mod
