...
unsigned const ASTEROID_NDIV = 32; // better if a power of 2
...
void free_obj::add_gravity(vector3d const &gravity, float gscale, bool near_bh) {
	...
	if (gravity.mag() > 0.95*BLACK_HOLE_GRAV) { // black hole does collision damage
...
vector<float> uobj_asteroid::pmap_vector; // static


uobj_asteroid::uobj_asteroid(point const &pos_, float radius_, unsigned model_, unsigned lt) :
	stationary_obj(SO_ASTEROID, pos_, radius_, lt), model(model_), scale_val(1.0)
{
	static int obj_id(0); // for random seed
	++obj_id;
	
	switch (model) {
		case AS_MODEL_SPHERE: // simple sphere - nothing to be done
			break;
		case AS_MODEL_ROCK1:
		case AS_MODEL_ROCK2: // rock_shape3d
			model3d.gen_rock(24, 1.0, obj_id, (model == AS_MODEL_ROCK2)); // pos starts at and stays at all_zeros
...
void uobj_asteroid::get_tex_coords_at(point const &query_pos, int &tx, int &ty) const {
...
float uobj_asteroid::damage(float val, int type, point const &hit_pos, free_obj const *source, int wc) {

	bool gen_fragments(model == AS_MODEL_HMAP && type == DAMAGE_EXP && val >= 20.0 && hit_pos != all_zeros && hit_pos != pos);
	...
	return stationary_obj::damage(val, type, hit_pos, source, wc);
}
...
float us_projectile::damage(float val, int type, point const &hit_pos, free_obj const *source, int wc) {
	...
	if (specs().bradius > 0.0) {
		...
		if (sobj_coll && damage_done >= 20.0) { // exploded on a planet or moon
			...
			gen_moving_fragments(hit_pos2, min(25U, max(1U, unsigned(min(10.0f, 0.4f*damage_done)))), 3.2, 6.0);
		}
	}
