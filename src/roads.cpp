...
void road_draw_state_t::pre_draw(vector3d const &xlate_, bool use_dlights_, bool shadow_only, bool always_setup_shader, bool enable_occlusion, bool enable_reflect) {
	draw_state_t::pre_draw(xlate_, use_dlights_, shadow_only, always_setup_shader, 0, enable_occlusion, enable_reflect); // enable_animations=0
...
void road_draw_state_t::draw_tunnel(tunnel_t const &tunnel, bool shadow_only) { // Note: called rarely, so doesn't need to be efficient
	...
	if (!shadow_only) {
		...
		select_texture_nmap(get_texture_by_name("normal_maps/cblock2_NRM.jpg", 1)); // set normal map
		tscale *= 4.0;
	}
