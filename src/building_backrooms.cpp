...
void building_t::add_backrooms_objs(rand_gen_t rgen, room_t &room, float zval, unsigned room_id, unsigned floor_ix, vect_cube_t &rooms_to_light) {
	...
	float const tot_light_amt(0.75); // no windows and not a single room; use a light amount less than max since it's in the basement
	...
	unsigned const custom_tid(interior->backrooms_tid); // store in the item_flags field for drawing and indir lighting
	...
	for (vect_cube_t &group : space_groups) {
		...
		for (auto s = group.begin(); s != group.end(); ++s) {
			...
			objs.emplace_back(pillar, TYPE_PG_PILLAR, room_id, 0, 0, RO_FLAG_BACKROOM, tot_light_amt, pillar_shape, wall_color, custom_tid); // dim=0, dir=0
	...
	for (unsigned d = 0; d < 2; ++d) {
		...
		for (cube_t &wall : walls_per_dim[d]) {
			objs.emplace_back(wall, TYPE_PG_WALL, room_id, d, 0, RO_FLAG_BACKROOM, tot_light_amt, SHAPE_CUBE, wall_color, custom_tid); // dir=0
	...
	room_object_t const &ref_light, vect_cube_t const &rooms_to_light, light_ix_assign_t &light_ix_assign)
{
	...
	for (cube_t const &r : rooms_to_light) {
		...
		add_sub_room_light(light, room_t(room, r), room_id, room_dim, objs_start, light_ix_assign, rgen);
	} // for r
}
void building_t::add_sub_room_light(room_object_t light, room_t const &room, unsigned room_id, bool dim, unsigned objs_start,
	light_ix_assign_t &light_ix_assign, rand_gen_t &rgen)
{
	vect_cube_t to_add;
	try_place_light_on_ceiling(light, room, room_id, dim, get_fc_thickness(), 1, 0, 1, 1, objs_start, to_add, rgen); // or wall light?
