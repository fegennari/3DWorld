...
class building_texture_mgr_t {
	...
	int vent_tid=-1, marble_floor_tid=-1, granite_floor_tid=-1, corr_metal_tid=-1, corr_metal_nm_tid=-1, br_wall_tid=-1, br_floor_tid=-1, fan_tid=-1, metal_vent_tid=-1;
...
public:
	...
	int get_metal_vent_tid   () {return ensure_tid(metal_vent_tid,    "metals/203_industrial_iron_metal_plate.jpg");}
...
public:
	...
	void init() {
		...
		register_tid(building_texture_mgr.get_metal_vent_tid());
	...
	public:
		...
		void draw_geom_range(tid_nm_pair_dstate_t &state, bool shadow_only, vert_ix_pair const &vstart, vert_ix_pair const &vend) { // use VBO rendering
			...
			if (vstart.tix != vend.tix) { // this is empty over half the time; merging this with the quads draw call likely has little runtime effect
				...
				draw_arrays_wrapper(GL_TRIANGLES, (vstart.tix + tri_vbo_off), (vend.tix - vstart.tix));
			}
			if (tex.tid == FONT_TEXTURE_ID) {
...
public:
	...
	void add_circle(tid_nm_pair_t const &tex, colorRGBA const &color, float radius, unsigned ndiv, bool dir, point const &pos, float tscale_s=1.0, float tscale_t=1.0) {
		assert(radius > 0.0);
		color_wrapper const cw(color);
		norm_comp const n(dir ? plus_z : -plus_z);
		float const css((dir ? -1.0 : 1.0)*TWO_PI/(float)ndiv), sin_ds(sin(css)), cos_ds(cos(css));
		float sin_s(0.0), cos_s(1.0);
		auto &tri_verts(get_verts(tex, 1));
		vert_norm_comp_tc_color const center(pos, n, 0.5*tscale_s, 0.5*tscale_t, cw);
		static vect_vnctcc_t verts;
		verts.clear();

		for (unsigned S = 0; S < ndiv; ++S) {
			float const s(sin_s), c(cos_s), ts((1.0 + s)*0.5*tscale_s), tt((1.0 + c)*0.5*tscale_t);
			verts.emplace_back((pos + point(radius*s, radius*c, 0.0)), n, ts, tt, cw);
			sin_s = s*cos_ds + c*sin_ds;
			cos_s = c*cos_ds - s*sin_ds;
		}
		for (unsigned S = 0; S < ndiv; ++S) { // no indices; 3 points per triangle
			tri_verts.push_back(center);
			tri_verts.push_back(verts[S]);
			tri_verts.push_back(verts[(S+1)%ndiv]);
		}
	}
	...
	void add_cooling_tower(building_t const &bg, cube_t const &ct) {
		...
		tid_nm_pair_t const cylin_tex(building_texture_mgr.get_met_plate_tid (), building_texture_mgr.get_mplate_nm_tid(), 1.0, 1.0);
		tid_nm_pair_t const top_tex  (cylin_tex.get_scaled_version(5.0));
		...
		base.z2() = tower.z1() = ct.z1() + 0.7*ct.dz();
		add_cube(bg, base, side_tex, WHITE, 0, 3, 1, 1); // draw sides
		add_cube(bg, base, top_tex,  WHITE, 0, 4, 1, 0); // draw top
...
void building_t::cut_holes_for_ext_doors(building_draw_t &bdraw, point const &contain_pt, unsigned draw_parts_mask, vect_cube_t const &clamp_cubes) const {
	if (doors.empty() || is_parking()) return;
	...
	for (auto d = doors.begin(); d != doors.end(); ++d) { // cut a hole for each door
		...
		//clip_door_to_interior(door); // not needed, since door frame blocks the edges; omitting this also fixes the wall edges that are visible at some angles
		bdraw.add_tquad(*this, door, bcube, tid_nm_pair_t(WHITE_TEX), WHITE);
	} // for d
}

bool building_t::get_nearby_ext_door_verts(building_draw_t *bdraw, shader_t &s, point const &pos, vector3d const &view_dir, float dist, bool only_open) {
...
bool building_t::get_all_nearby_ext_door_verts(building_draw_t &bdraw, shader_t &s, vector<point> const &pts, float dist) { // for pedestrians
	for (auto const &p : pts) {
		...
		if (get_nearby_ext_door_verts(&bdraw, s, p, zero_vector, dist, 1)) return 1; // no view_dir, only_open=1
	}
	return 0;
}
				...
				for (auto g = bc->grid_by_tile.begin(); g != bc->grid_by_tile.end(); ++g) {
					...
					for (auto bi = g->bc_ixs.begin(); bi != g->bc_ixs.end(); ++bi) {
						...
						if (camera_in_this_building && b.has_house_floorplan()) { // check for shadows from open exterior doors that open inward
							b.get_nearby_ext_door_verts(nullptr, s, pre_smap_player_pos, cview_dir, get_door_open_dist(), 1); // no door_draw, only_open=1
						}
			...
			for (auto i = bcs.begin(); i != bcs.end(); ++i) { // draw only nearby interiors
				...
				for (auto g = (*i)->grid_by_tile.begin(); g != (*i)->grid_by_tile.end(); ++g) { // Note: all grids should be nonempty
					...
					for (cube_with_ix_t const &bi : g->bc_ixs) {
						...
						bool const had_open_door(b.get_nearby_ext_door_verts(&ext_door_draw, s, camera_bs, cview_dir, door_open_dist, 0)); // only_open=0
						...
						this_frame_player_in_attic    =  (b.point_in_attic(camera_bs) ? (b.attic_has_window_or_skylight() ? 1 : 2) : 0);
