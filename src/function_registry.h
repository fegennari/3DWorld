...
void gen_cylinder_triangle_strip(vector<vert_norm_tc> &verts, vector_point_norm const &vpn,
	bool two_sided_lighting=0, float tc_t0=0.0, float tc_t1=1.0, float ts_scale=1.0, vector3d const &xlate=zero_vector);
void gen_cylinder_quads(vector<vert_norm_tc> &verts, vector_point_norm const &vpn, bool two_sided_lighting=0, float tex_scale_len=1.0, float tex_len_off=0.0);
...
float get_exact_zval(float xval, float yval, bool no_xyoff=0);
void reset_offsets();
...
bool line_segs_intersect_2d(vector2d const &L1a, vector2d const &L1b, vector2d const &L2a, vector2d const &L2b, vector2d *p_int=nullptr);
