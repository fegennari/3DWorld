	...
	if (inc_sm) { // add wall trim for each side of the closet door
		...
		for (unsigned is_side = 0; is_side < 2; ++is_side) { // front wall, side wall
			for (unsigned d = 0; d < 2; ++d) {
				...
				get_material(tid_nm_pair_t(), 0, 0, 1).add_cube_to_verts_untextured(btrim, trim_color, skip_faces); // is_small, untextured, no shadows; both interior and exterior
...
void building_room_geom_t::add_sign(room_object_t const &c, bool inc_back, bool inc_text) {
	if (inc_back) {
		...
		get_material(tid_nm_pair_t(), 0).add_cube_to_verts_untextured(c, WHITE, skip_faces); // back of the sign, always white (for now)
	}
	if (!inc_text) return;
	// add sign text
	...
	unsigned building_ix, bool shadow_only, bool reflection_pass, bool inc_small, bool player_in_building)
{
	...
	if (!shadow_only) { // alpha blended, should be drawn near last
		draw_building_interior_spraypaint(3, &building); // draw both interior and exterior
		draw_building_interior_decals(&building); // for blood decals
	}
