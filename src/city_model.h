...
enum {ANIM_ID_NONE=0, ANIM_ID_WALK, ANIM_ID_BHOP, ANIM_ID_FLIP, ANIM_ID_TWIRL, ANIM_ID_MARCH, ANIM_ID_ALIEN,
	  ANIM_ID_RAT, ANIM_ID_SPIDER, ANIM_ID_SKELETAL, ANIM_ID_HCOPTER, ANIM_ID_SWINGS, ANIM_ID_FISH_TAIL, ANIM_ID_FLAP_ARMS, ANIM_ID_WIND_TUR};
...
public:
	...
	void draw_model(shader_t &s, vector3d const &pos, cube_t const &obj_bcube, vector3d const &dir, colorRGBA const &color, vector3d const &xlate,
		unsigned model_id, bool is_shadow_pass=0, bool low_detail=0, animation_state_t *anim_state=nullptr, unsigned skip_mat_mask=0, bool untextured=0,
		bool force_high_detail=0, bool upside_down=0, bool emissive=0, bool do_local_rotate=0, int mirror_dim=3, bool using_custom_tid=0, int swap_xy_mode=0);
...
public:
	...
	virtual city_model_t const &get_model(unsigned id) const;
	virtual city_model_t       &get_model(unsigned id);
	int select_random_model(int rand_val, bool choose_zombie, unsigned pref_gender, bool pref_prisoner);
	...
	bool is_prisoner_model(unsigned id) const;
};
