	...
	if (using_tiled_terrain_hmap_tex() && !using_hmap_with_detail()) { // optimized flow when using heightmap texture; not adding xoff2/yoff2
		...
		for (float y = c.y1()-y_step; y < c.y2()+y_step; y += y_step) {
			for (float x = c.x1()-x_step; x < c.x2()+x_step; x += x_step) {
				min_eq(hmin, get_exact_zval(min(x, c.x2()), min(y, c.y2()), 1)); // check every grid point with the X/Y range; no_xyoff=1
				if (hmin < stop_at) return hmin;
			}
		}
	}
...
bool building_t::add_underground_exterior_rooms(rand_gen_t &rgen, cube_t const &door_bcube, cube_t const &basement, bool wall_dim, bool wall_dir, float length_mult) {
	...
	add_secret_underground_rooms(P, rgen);
	...
	for (ceiling_space_t const &cs : interior->ceiling_spaces) {
		...
		for (; cur_obj_ix != objs_end && objs[cur_obj_ix].room_id <= cs.room_ix; ++cur_obj_ix) {
			...
			if (rand_val == 0) { // make the light fall
				...
				light.dir    = dir;
				light.flags |= RO_FLAG_ROTATING; // flag as rotated/hanging
...
void building_t::add_missing_wall_objects(rand_gen_t rgen) {
	...
	for (wall_seg_t const &w : interior->missing_wall_segs) {
		unsigned const flags(w.open ? (RO_FLAG_OPEN | RO_FLAG_NOCOLL) : 0);
		interior->room_geom->objs.emplace_back(w, TYPE_WALL_GAP, w.room_ix, w.dim, w.dir, flags, 1.0, SHAPE_CUBE, WHITE, rgen.rand()); // random item flags
	}
}
...
bool building_t::add_cave_objs(rand_gen_t rgen, room_t const &room, float zval, unsigned room_id) {
	...
	add_room_wall_tile(room, room_id, light_amt, TYPE_ROCK_WALL);
	// add random vertical walls + blockers inside the room?
	return 1;
}
