	...
	float side_tscale_add, bool swap_txy, float len_tc2, float len_tc1, int half_or_quarter, int swap_end_txy)
{
	...
	if (!skip_sides) {
		...
		else if (side_tscale == 0.0) { // untextured
			for (unsigned i = 0; i <= ndiv_draw; ++i) { // vertex data
				unsigned const s(i%ndiv);
				norm_comp const normal(0.5*(vpn.n[s] + vpn.n[(i+ndiv-1)%ndiv])); // normalize?
				itri_verts[itix++].assign(vpn.p[(s<<1)+0], normal, 0.0, 0.0, cw);
				itri_verts[itix++].assign(vpn.p[(s<<1)+1], normal, 0.0, 0.0, cw);
			}
		}
		else {
			for (unsigned i = 0; i <= ndiv_draw; ++i) { // vertex data
...
void rgeom_mat_t::add_round_rect_to_verts(cube_t const &c, float corner_radius, colorRGBA const &color, bool draw_top,
	bool draw_bot, bool skip_sides, bool two_sided, float rs_bot, float rs_top, unsigned ndiv)
{
	...
	add_vcylin_to_verts(corner, color, draw_bot, draw_top, two_sided, 0, rs_bot, rs_top, 0.0, 0.0, skip_sides); // untextured; set side_tscale=end_tscale=0.0
