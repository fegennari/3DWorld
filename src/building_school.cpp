...
bool building_t::add_library_objs(rand_gen_t rgen, room_t const &room, float &zval, unsigned room_id, float tot_light_amt, unsigned objs_start, bool is_basement, bool add_tables) {
	...
	if (is_prison() || is_school() || is_hospital()) {zval = add_flooring(room, zval, room_id, tot_light_amt, FLOORING_CARPET);} // if floor is tile; after bookcases?
...
{
	bool const is_rest(room.is_restaurant()); // only need to check ground floor for restaurants
	float const vspace(get_window_vspace()), clearance(get_min_front_clearance_inc_people());
	float const table_spacing(0.5*vspace + (is_rest ? 2.25 : 2.0)*clearance); // placed tables may be rectangular, but we use square spacing; more spacing for restaurants
	...
	for (unsigned y = 0; y < ny; ++y) {
		for (unsigned x = 0; x < nx; ++x) {
			...
			// 4 chairs, short table
			num_added += add_table_and_chairs(rgen, room, blockers, room_id, center, chair_color, 0.0, tot_light_amt, 4, 0, plastic_tc, chair_rand_add, is_rest);
		}
	} // for y
			...
			if (is_prison() && rgen.rand_float() < 0.33) { // maybe add cigarettes; add an ashtray as well?
				...
				if (num_cig > 0 && max_len < 0.5*min(surface.dx(), surface.dy())) { // should always be true
					for (unsigned n = 0; n < num_cig; ++n) {
						...
						objs.emplace_back(cig, TYPE_CIGARETTE, room_id, dim, rgen.rand_bool(), RO_FLAG_NOCOLL, tot_light_amt, SHAPE_CYLIN, WHITE);
					} // for n
				}
			}
		}
