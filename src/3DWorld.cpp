...
bool no_smoke_over_mesh(0), enable_model3d_tex_comp(0), recalc_model3d_normals(0), write_model3d_file(0), player_near_fire(0);
bool global_lighting_update(0);
...
int load_config(string const &config_file) {
	...
	while (read_str(fp, strc)) { // slow but should be OK
		...
		else if (str == "write_model3d_file") {
			if (!read_bool(fp, write_model3d_file)) cfg_err("write_model3d_file command", error);
		}
