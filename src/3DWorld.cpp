...
bool enable_dpart_shadows(0), enable_tt_model_reflect(1), enable_tt_model_indir(0), auto_calc_tt_model_zvals(0), use_model_lod_blocks(0), enable_translocator(0);
...
float model_mat_lod_thresh(5.0), clouds_per_tile(0.5), def_atmosphere(1.0), def_vegetation(1.0);
...
void move_camera_pos_xy(vector3d const &v, float dist) {
	...
	proc_buildings_sphere_coll(surface_pos, prev, CAMERA_RADIUS, 0); // use prev pos for building collisions
}
...
void keyboard_proc(unsigned char key, int x, int y) {
	...
	case '9': // toggle leaf wind, ocean waves, footsteps, snow footprints, and asteroid belt fog
