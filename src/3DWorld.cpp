...
bool univ_planet_lod(0), tiled_mesh_display(1);
...
int star_init(0), show_lightning(0), recreated(1), timer_a(0), stencil_shadow(0);
...
int litning_dynamic(0), shadow_detail(DEF_SD), do_zoom(0), disable_universe(0), disable_inf_terrain(0);
...
float subdiv_size_mult(2.0), light_int_scale(1.0), light_int_scale_l(1.0), snow_depth(0.0), snow_random(0.0);
...
void advance_camera(int dir) {
	...
	if (world_mode == WMODE_UNIVERSE) { // universe
		...
		if (game_mode && sstates != NULL && sstates[CAMERA_ID].powerup == PU_SPEED) dist *= 1.5;
...
void keyboard_proc(unsigned char key, int x, int y) {
	...
	case 'l': // enable lightning / dock fighters
		...
		show_lightning  = (show_lightning+1)%3;
		litning_dynamic = (show_lightning == 2);
		break;
...
void keyboard2(int key, int x, int y) {
	...
	case GLUT_KEY_F6: // use new long-range tiled_mesh_display mode
		tiled_mesh_display ^= 1;
		clear_tiled_terrain();
		break;
...
int load_config(string const &config_file) {
	...
	while (read_str(fp, strc)) { // slow but should be OK
		...
		else if (str == "subdiv_size_mult") {
			if (!read_float(fp, subdiv_size_mult) || subdiv_size_mult <= 0.0) cfg_err("subdiv_size_mult command", error);
		}
