...
void building_t::gather_interior_cubes(vect_colored_cube_t &cc, cube_t const &ext_bcube) const {
	...
	for (auto c = objs.begin(); c != objs.end(); ++c) {
		...
		if (c->type == TYPE_WALL_TRIM || c->type == TYPE_BOOK || c->type == TYPE_CRACK || c->type == TYPE_PLANT || c->type == TYPE_RAILING || c->type == TYPE_SHELVES ||
			...
			c->type == TYPE_PLANT || c->type == TYPE_WALL_LAMP || c->type == TYPE_RCHAIR || c->type == TYPE_SILVER || c->type == TYPE_STAPLER || c->type == TYPE_WIND_SILL ||
	...
	for (auto i = objs.begin(); i != objs_end; ++i) {
		...
		if (camera_surf_collide && (camera_in_building || camera_can_see_ext_basement) && dist_less_than(lpos_rot, camera_bs, /*dshadow_radius*/light_radius)) {
			...
			if (clipped_bc.contains_pt(camera_rot) || clipped_bc.contains_pt(point(camera_rot.x, camera_rot.y, player_feet_zval)) || player_on_ladder_this_room) {
				...
				bool const check_floor_above(camera_on_stairs || (camera_by_stairs && camera_room == (int)i->room_id));
