...
class cube_bvh_t : public cobj_tree_simple_type_t<colored_cube_t> {
	...
	virtual void refine_split_pos(tree_node const &n, unsigned skip_dims, unsigned &dim, float &sval) override { // SAH split; ~4% faster
		...
		cube_t left_bbox(objects[n.start]), right_bbox(objects[n.end-1]); // seed with first/last object
		right_costs.resize(n.size()-1);
		
		for (unsigned i = n.end-1; i > n.start; --i) { // iterate backwards
			right_bbox.union_with_cube(objects[i]);
		...
		for (unsigned i = n.start+1; i < n.end; ++i) {
			...
			left_bbox.union_with_cube(objects[i]); // add object to left bbox once we've passed it; left_bbox always increases
		}
...
bool building_t::ray_cast_interior(point const &pos, vector3d const &dir, ray_cast_args_t const &args, point &cpos, vector3d &cnorm, colorRGBA &ccolor, rand_gen_t *rgen) const {
	if (!interior || is_rotated()) return 0; // these cases are not yet supported
	point p1(pos), p2(pos + dir*(2.0*args.max_extent));
	if (!do_line_clip(p1, p2, args.valid_area.d)) return 0; // ray does not intersect clip cube
	...
	// check parts (exterior walls); should chimneys and porch roofs be included?
		...
		if (hit) { // exterior hit (ray outside building) - don't need to check interior geometry
			cpos   = p1 + (p2 - p1)*t;
			ccolor = args.bcolors.side_color;
			return 1;
		}
	}
...
void building_t::gather_interior_cubes(vect_colored_cube_t &cc, cube_t const &ext_bcube) const {
	...
	for (auto c = objs.begin(); c != objs.end(); ++c) { // Note: ignores expanded objects (including shelf rack objects)
		...
		if (type == TYPE_WALL_TRIM || type == TYPE_BOOK || type == TYPE_PLANT || type == TYPE_RAILING || type == TYPE_BUCKET || type == TYPE_LBASKET || c->is_a_drink() ||
			...
			type == TYPE_MILK || type == TYPE_TRASH || type == TYPE_PLATE || type == TYPE_SPRINKLER || type == TYPE_TOASTER || type == TYPE_FOOD_TUB || type == TYPE_KEYBOARD ||
			type == TYPE_LAPTOP || type == TYPE_VASE || type == TYPE_URN || type == TYPE_LAVALAMP || type == TYPE_TOY || type == TYPE_EASEL || type == TYPE_GENERATOR ||
			type == TYPE_TV_REMOTE || type == TYPE_WETF_SIGN || type == TYPE_POOL_BALL) continue;
		...
		if (c->is_round()) {
		...
		else if (type == TYPE_BED) { // Note: posts and bunk bed ladders are not included
			colorRGBA const frame_color (c->in_jail() ? BKGRAY : get_textured_wood_color());
			colorRGBA const sheets_color(c->color.modulate_with(texture_color(c->get_sheet_tid())));
			cube_t cubes[6]; // frame, head, foot, mattress, pillow, legs_bcube
			get_bed_cubes(*c, cubes);
			add_colored_cubes(cubes,   3, frame_color,  cc); // frame, head, foot
			...
			add_colored_cubes(cubes, 4, frame_color, cc); // legs
		}
		else if (type == TYPE_DESK || type == TYPE_DRESSER || type == TYPE_NIGHTSTAND || type == TYPE_TABLE) { // objects with legs
			cube_t cubes[7];
			unsigned num(get_table_like_object_cubes(*c, cubes));
			colorRGBA legs_color(color);
			if (c->is_glass_table()) {swap(cubes[0], cubes[num-1]); --num; legs_color = BLACK;} // skip glass table, skip the top
			add_colored_cubes(cubes, num, legs_color, cc);
		}
			...
			cc.emplace_back(cubes[1], texture_color(WOOD2_TEX)); // base
		}
		...
		// handle open medicine cabinets and lockers; should they always be treated as open? should the open doors be included?
		...
		else if (type == TYPE_SINK) {
			cube_t top(*c), bot(*c);
			top.z2() -= 0.20*c->dz();
			top.z1() += 0.65*c->dz();
			bot.z2() = top.z1();
			for (unsigned d = 0; d < 2; ++d) {set_wall_width(bot, c->get_center_dim(d), 0.16*c->get_depth(), d);}
			cc.emplace_back(top, color);
			cc.emplace_back(bot, color);
		}
		else if (type == TYPE_SHELVES) {
			if (c->in_warehouse()) continue; // metal grid, skip
			cube_t shelves[MAX_SHELVES];
			add_colored_cubes(shelves, get_shelves_for_object(*c, shelves), color, cc);
		}
		else if (type == TYPE_BENCH) {
			cube_t cubes[4]; // seat, leg, leg, [back]
			unsigned const num(get_bench_cubes(*c, cubes));

			if (c->item_flags != 1) { // add top and maybe back; skip for mesh bench
				cc.emplace_back(cubes[0], color);
				if (num == 4) {cc.emplace_back(cubes[3], color);}
			}
			...
			cube_t cubes[3];
			get_shower_tile_cubes(*c, cubes);
			add_colored_cubes(cubes, 3, get_shower_tile_color(*c), cc);
		}
		else if (type == TYPE_SHOWERTUB) { // includes wall and tile, but not curtains since they're dynamic
			cube_t const wall(get_shower_tub_wall(*c));
			cc.emplace_back(wall, color);
			float const thickness(0.1*wall.get_sz_dim(!dim));
			cube_t const tiled_area(get_shower_tub_tiled_area(*c, wall));
			cube_t sides[3] = {tiled_area, tiled_area, tiled_area}; // side, side, back
			...
			cube_t body_back(body);
			body_back.d[dim][dir] = c->d[dim][!dir] + (dir ? 1.0 : -1.0)*wall_width; // thin back wall
			cc.emplace_back(bot, color);
			cc.emplace_back(top, color);
...
class building_indir_light_mgr_t {
	...
	void cast_light_rays(building_t const &b) {
		...
		if (is_window) { // window
			...
			else if (ro.flags & (RO_FLAG_ADJ_HI | RO_FLAG_ROTATING)) {dim = ro.dim; dir = ro.dir;} // wall light or rotated/hanging
...
#pragma omp parallel for schedule(dynamic) num_threads(num_rt_threads)
		for (int n = 0; n < num_rays; ++n) {
			...
			bool const hit(b.ray_cast_interior(origin, pri_dir, args, init_cpos, init_cnorm, ccolor, &rgen));
...
public:
	...
	void register_cur_building(building_t const &b, unsigned bix, point const &target, unsigned &tid) { // target is in building space
		...
		if ((int)bix != cur_bix) { // change to a different building; or is it better to store a building_t const *?
			clear();
			cur_bix = bix;
			assert(!is_running);
			build_bvh(b, target);
	...
	void register_light_state_change(unsigned light_ix, bool light_is_on, bool in_elevator, bool geom_changed) {
		...
		bool const is_cur_light((int)light_ix == cur_job.lix && is_running);
	...
	occlusion_checker_noncity_t const &oc, vect_cube_with_ix_t &ped_bcubes, cube_t &lights_bcube)
{
	...
	for (auto i = objs.begin(); i != objs_end; ++i) {
		...
		bool const hanging(!wall_light && !is_lamp && (i->flags & RO_FLAG_ROTATING)); // rotated/broken light
