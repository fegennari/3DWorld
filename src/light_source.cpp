...
cube_t light_source::calc_bcube(bool add_pad, float sqrt_thresh, bool clip_to_scene_bcube, float falloff) const {
	...
	if (has_custom_bcube()) {
		if (!bcube.intersects(custom_bcube)) {
			cout << TXTS(pos) << TXTS(pos2) << TXT(radius) << TXT(sqrt_thresh) << TXTS(bcube) << TXTS(custom_bcube) << endl;
			assert(0);
		}
...
bool light_source::is_visible() const {
	...
	for (unsigned n = 0; n < num_rays; ++n) { // for static scene lights we do ray queries
		...
		else { // not found in cache, compute intersection point and add it
...
void light_source::draw_light_cone(shader_t &shader, float alpha) const {
	...
	for (unsigned n = 0; n < nstacks; ++n) { // stacks
		...
		vector_point_norm const &vpn(gen_cylinder_data((cylin.p1 + v2*dir), (cylin.p1 + v1*dir), v2*cylin.r2, v1*cylin.r2, ndiv)); // swap points to make second point the tip/zero radius
