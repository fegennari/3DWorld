...
void draw_circle_normal(float r_inner, float r_outer, int ndiv, int invert_normals, point const &pos, float tscale_s, float tscale_t) {
	...
	for (unsigned S = 0; S <= (unsigned)ndiv; ++S) {
		...
		if (disk) {verts.emplace_back((pos + point(r_inner*s, r_inner*c, 0.0)), n, (1.0f + inner_tscale*s)*half_tss, (1.0f + inner_tscale*c)*half_tst);}
...
void gen_cylinder_quads(vector<vert_norm_tc> &verts, vector_point_norm const &vpn, bool two_sided_lighting, float tex_scale_len, float tex_len_off) {
	...
	for (unsigned i = 0; i < ndiv; ++i) { // Note: always has tex coords
		for (unsigned j = 0; j < 2; ++j) {
			...
			vector3d const normal(vpn.n[s] + vpn.n[(S+ndiv-1)%ndiv]); // normalize?
			create_vert(verts[vix++], vpn.p[(s<<1)+ j], normal, ts, (tex_len_off + tex_scale_len*( j)), two_sided_lighting);
			create_vert(verts[vix++], vpn.p[(s<<1)+!j], normal, ts, (tex_len_off + tex_scale_len*(!j)), two_sided_lighting);
		}
	} // for i
...
void sd_sphere_vbo_d::draw_ndiv_pow2_vbo(unsigned draw_ndiv) {
	...
	if (faceted) {draw_arrays_wrapper(GL_TRIANGLES, 0, 6*ndiv*ndiv);} // ndiv is ignored
	else {glDrawRangeElements(GL_TRIANGLE_STRIP, 0, (ndiv+1)*(ndiv+1), get_count(lod), get_index_type_enum(), get_index_ptr(lod)); ++num_frame_draw_calls;}
	post_render();
}
...
void draw_sphere_vbo_pre_bound(int ndiv, bool textured, bool half, unsigned num_instances) {
	...
	assert(off1 < off2);
	check_mvm_update();
	draw_arrays_wrapper(GL_TRIANGLE_STRIP, off1, (off2 - off1), num_instances); // uses triangle strips separated by degenerate triangles
}
